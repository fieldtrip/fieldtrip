...
elseif ft_filetype(filename, 'loreta_lorb')
  ...
  error(defaultId, 'unsupported LORETA format');
end
...
if ~is_txt
  ...
  elseif length(timeframe)==1
    ...
    error(defaultId, 'you can read either one timeframe, or the complete timecourse');
  end
  fclose(fid);
else
  % read with textfile
  ...
  if size(activity,1) == voxnumber || size(activity,2) == voxnumber
    ...
    error(defaultId, 'expect column or row to be length 2394 or 6239')
  end
  if isempty(timeframe)
  else
    % read timeframe
