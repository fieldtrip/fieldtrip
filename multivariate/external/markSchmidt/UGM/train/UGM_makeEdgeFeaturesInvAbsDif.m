function [Xedge] = UGM_makeEdgeFeaturesInvAbsDif(X,edgeEnds,globalFeatures)
...
for i = 1:nInstances
    for e = 1:nEdges
        ...
        elseif existGF
            ...
            Xedge(i,:,e) = [X(i,find(globalFeatures==1),n1) 1./(1+abs(X(i,find(globalFeatures==0),n1)-X(i,find(globalFeatures==0),n2)))];
        else
            % Only local features
            Xedge(i,:,e) = 1./(1+abs(X(i,:,n1)-X(i,:,n2)));
        end
    end
end