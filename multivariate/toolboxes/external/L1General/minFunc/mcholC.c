...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    *c,    *H, *L, *D, *A;
    ...
    for (i = 0; i < n; i++)
    {
        gamma = max(gamma,absolute(A[i+n*i]));
        for (j = 0;j < n; j++)
        {
            if (i != j)
                xi = max(xi,absolute(A[i+n*j]));
        }
    }
    ...
    if (n > 1)
    {
        beta = sqrt(max(gamma,max(mu,xi/sqrt(n*n-1))));
    }
    else
    {
    ...
    /* Find q that results in Best Permutation with j */
        ...
        for(i = j; i < n; i++)
        {
            ...
            {
                maxVal = max(maxVal,absolute(c[i+n*i]));
                q = i;
            }
        }
        ...
        {
            for(i = j+1;i < n; i++)
                theta = max(theta,absolute(c[i+n*j]));
        }
        
        D[j] = max(absolute(c[j+n*j]),max(delta,theta*theta/(beta*beta)));
