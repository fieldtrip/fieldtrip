...
elseif ischar(cfg.trials)
  error(defaultId, 'only ''all'' is allowed for string input for cfg.trials');
else
  ...
  if isfield(source, 'trial')
    ...
    error(defaultId, 'subselecting trials in ft_sourcedescriptives is currently only possible with a ''trial'' field');   
  end
end
...
if strcmp(cfg.projectmom, 'yes')
  ...
  elseif ~strcmp(cfg.powmethod, 'regular')
    error(defaultId, 'unsupported powmethod in combination with projectmom');
  end
  if isempty(cfg.cohmethod)
    cfg.cohmethod = 'regular';% set the default
  elseif ~strcmp(cfg.cohmethod, 'regular')
    error(defaultId, 'unsupported cohmethod in combination with projectmom');
  end
else
  if isempty(cfg.powmethod)
...
if strcmp(cfg.resolutionmatrix, 'yes')
  if ~isfield(source.avg, 'filter')
    error(defaultId, 'The computation of the resolution matrix requires keepfilter=''yes'' in sourceanalysis.');
  elseif ~isfield(source, 'leadfield')
    error(defaultId, 'The computation of the resolution matrix requires keepleadfield=''yes'' in sourceanalysis.');
  end
end

if strcmp(cfg.fwhm, 'yes')
  if ~isfield(source.avg, 'filter')
    error(defaultId, 'The computation of the fwhm requires keepfilter=''yes'' in sourceanalysis.');
  end
end

if strcmp(cfg.eta, 'yes') && strcmp(cfg.cohmethod, 'svdfft')
  error(defaultId, 'eta cannot be computed in combination with the application of svdfft');
end

if strcmp(cfg.keeptrials, 'yes') && ~strcmp(cfg.supmethod, 'none')
  error(defaultId, 'you cannot keep trials when you want to partialize something');
end
...
switch cfg.powmethod
  ...
  case 'none'
    powmethodfun = [];
  otherwise
    error(defaultId, 'unsupported powmethod');
end
...
if ispccdata
  ...
  if numel(refchansel)>0 && numel(refdipsel)>0
    error(defaultId, 'cannot simultaneously handle reference channels and reference dipole');
  end
  ...
  if keeptrials
    ...
    if ~strcmp(cfg.supmethod, 'none')
      error(defaultId, 'suppression is only supported for average CSD');
    end
    ...
    for i=insideindx
      ...
      if ~isempty(refsel)
        ...
        switch cfg.cohmethod
          ...
          otherwise
            error(defaultId, 'unsupported cohmethod');
        end % cohmethod
      end
  ...
  if strcmp(cfg.kurtosis, 'yes')
    ...
    for diplop=1:length(insideindx)
      ...
      if length(mom)~=prod(size(mom))
        error(defaultId, 'kurtosis can only be computed for projected dipole moment');
      end
...
if strcmp(source.method, 'jackknife') || strcmp(source.method, 'bootstrap') || strcmp(source.method, 'pseudovalue') || strcmp(source.method, 'singletrial') || strcmp(source.method, 'rawtrial')
  ...
  elseif strcmp(source.method, 'pseudovalue')
    ...
    warning(defaultId, 'don''t know how to compute bias for pseudovalue resampling');
    bias = 1;
  end
  ...
  switch cfg.fwhmmethod
    case 'barnes'
      if ~isfield(source, 'dim')
        error(defaultId, 'computation of fwhm is not possible with method ''barnes'' is not possible when the dipoles are not defined on a regular 3D grid');
      end
    ...
    otherwise
      error(defaultId, 'unknown method for fwhm estimation');
  end
end
