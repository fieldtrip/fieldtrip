...
elseif numthreads>0 && isinf(numthreads)
  ...
  error(defaultId, 'unsupported number of threads (%d)', numthreads);
end
...
if ~isempty(toolbox)
  ...
  for i=1:length(toolbox)
    ...
    if isempty(toolboxpath)
      error(defaultId, 'the Mathworks toolbox "%s" could not be found', toolbox{i});
    else
      fprintf('including %s\n', toolboxpath);
    end
...
if ~isempty(executable)
  ...
  if ~exist(executable, 'file')
    error(defaultId, 'executable "%s" does not exist', executable);
  end
  fprintf('using "%s"\n', executable);
else
