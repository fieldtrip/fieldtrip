...
if isempty(jobid) && ~isempty(pbsid)
  ...
  if length(sel)==1
    ...
    warning(defaultId, 'cannot determine the jobid that corresponds to pbsid %s', pbsid);
  end
end
...
if isempty(pbsid) && ~isempty(jobid)
  ...
  if length(sel)==1
    ...
    warning(defaultId, 'cannot determine the pbsid that corresponds to jobid %s', jobid);
  end
end
...
switch cmd
      ...
      switch backend
        case 'torque'
          ...
          if isempty(jobstatus)
            warning(defaultId, 'cannot determine the status for pbsid %s', pbsid);
            retval = 1;
          else
  ...
  otherwise
    error(defaultId, 'unsupported command (%s)', cmd);
end % switch

if length(list_jobid)~=length(list_pbsid)
  error(defaultId, 'jobid and pbsid lists are inconsistent');
end
