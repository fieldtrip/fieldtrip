...
try
  ...
  while (~exist(inputfile, 'file') && toc(stopwatch)<60)
    ...
    warning(defaultId, 'the input file %s does not yet exist', inputfile);
    pausejava(10);
  end
  clear stopwatch
  
  if ~exist(inputfile, 'file')
    error(defaultId, 'timeout while waiting for the input file %s', inputfile);
  end
  ...
  if isequal(lastid, 'MATLAB:dispatcher:UnresolvedFunctionHandle')
    ...
    if ~isempty(whichfunction) && exist(whichfunction, 'file')
      warning(defaultId, 'assuming %s as full function name', whichfunction);
      oldpwd = pwd;
  ...
  if retval~=0
    error(defaultId, 'problem renaming output file %s', outputfile);
  end
  ...
  disp(err);
  warning(defaultId, 'an error was caught');
