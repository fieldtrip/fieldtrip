...
if isempty(val) && ~any(strcmp(allowedtype, 'empty'))
  if isnan(ft_getopt(opt, key, nan))
    error(defaultId, 'the option "%s" was not specified or was empty', key);
  end
end
...
if ~ok
  if length(allowedtype)==1
    error(defaultId, 'the type of the option "%s" is invalid, it should be "%s" instead of "%s"', key, allowedtype{1}, valtype);
  else
    error(defaultId, 'the type of the option "%s" is invalid, it should be any of %s instead of "%s"', key, printcell(allowedtype), valtype);
  end
end
...
if ~ok
  error(defaultId, 'the value of the option "%s" is invalid', key);
end
