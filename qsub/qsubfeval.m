...
if ~compiled
  ...
  elseif isempty(previous_matlabcmd)
    ...
    if system(sprintf('which %s > /dev/null', matlabcmd))==1
      ...
      warning(defaultId, 'the executable for "%s" could not be found, trying "matlab" instead', matlabcmd);
...
switch backend
  ...
  case 'torque'
    ...
    if any(curPwd==' ')
      ...
      error(defaultId, 'you cannot execute jobs from within a directory that has a space in its name');
    end
  ...
  otherwise
    error(defaultId, 'unsupported backend "%s"', backend);
