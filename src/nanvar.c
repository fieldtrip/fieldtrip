    ...
    if (inputi_p == NULL)
      ...
      for (i=0; i<numelin; i++)
        ...
        for (i=0; i<numelout; i++)
          output1r_p[i] = (ssqr[i] - (output1r_p[i]*output1r_p[i])/cnt[i])/(cnt[i] - biasterm);
      }
    else
      ...
      /*compute the standard deviation from the running sum*/
      for (i=0; i<numelout; i++)
        output1r_p[i] = (ssqr[i] + ssqi[i] - (output1r_p[i]*output1r_p[i] + sumi[i]*sumi[i])/cnt[i])/(cnt[i] - biasterm);
      }    
  ...
  else if (classid==mxSINGLE_CLASS)
  {
    ...
    if (inputi_ps == NULL)
      ...
      for (i=0; i<numelin; i++)
        ...
        /*compute the standard deviation from the running sum*/
        for (i=0; i<numelout; i++)
          output1r_ps[i] = (ssqrs[i] - (output1r_ps[i]*output1r_ps[i])/cnts[i])/(cnts[i] - biasterms);
      }
    else
      /*handle the complex valued case separately*/
      {
      ...
      /*compute the standard deviation from the running sum*/
      for (i=0; i<numelout; i++)
        output1r_ps[i] = (ssqrs[i] + ssqis[i] - (output1r_ps[i]*output1r_ps[i] + sumis[i]*sumis[i])/cnts[i])/(cnts[i] - biasterms);
      }    
