...
if isempty(list)
  warning(defaultId, 'there is no peer available as slave, reverting to local cellfun');
...
while ~all(submitted) || ~all(collected)
  ...
  if ~isempty(submit) && (sum(submitted)-sum(collected))<MaxBusy
    ...
    ws = warning(defaultId, 'off', 'FieldTrip:peer:noSlaveAvailable');
  ...
  for i=1:numel(joblist)
    ...
    if isempty(collect) && ~isempty(resubmitted)
      ...
      if ~isempty(collect) && ~collected(collect)
        ...
        warning(defaultId, 'the original job %d did return, reverting to its original results', collect);
      end
    end
    ...
    try
      ...
      if ~isempty(strfind(peerget_err.message, 'could not start the MATLAB engine')) || ...
        ...
        if ~isempty(strfind(peerget_err.message, 'could not start the MATLAB engine'))
          warning(defaultId, 'resubmitting job %d because the MATLAB engine could not get a license', collect);
        end
  ...
  % for i=1:length(sel)
  %   warning(defaultId, 'job %d has not been seen for 60 seconds\n', sel(i));
  % end
  ...
  for i=1:length(sel)
    warning(defaultId, 'resubmitting job %d because it takes too long to get started', sel(i));
  ...
  for i=1:length(sel)
    warning(defaultId, 'resubmitting job %d because it takes too long to finish (estimated = %s)', sel(i), print_tim(estimated));
    % remember the job that will be resubmitted, it still might return its results
    resubmitted(end+1).jobnum = sel(i);
    resubmitted(end  ).jobid  = jobid(sel(i));
    resubmitted(end  ).time   = toc(stopwatch);
