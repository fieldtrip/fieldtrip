...
if ~isempty(previous_argin) && ~isequal(varargin{1}, previous_argin{1})
  ...
  if isempty(which(varargin{1}))
    error(defaultId, 'Not a valid M-file (%s).', varargin{1});
  end
end
...
while isempty(jobid)
  ...
  if isempty(list)
    error(defaultId, 'there is no peer available as slave');
  end
  
  % only peers with enough memory are interesting
  list = list([list.memavail] >= memreq);
  if isempty(list)
    error(defaultId, 'there are no slave peers available that meet the memory requirements');
  end
  
  % only peers with enough CPU speed are interesting
  list = list([list.cpuavail] >= cpureq);
  if isempty(list)
    error(defaultId, 'there are no slave peers available that meet the CPU requirements');
  end
  
  % only peers with enough time for a single job are interesting
  list = list([list.timavail] >= timreq);
  if isempty(list)
    error(defaultId, 'there are no slave peers available to execute a job of this duration');
  end
...
if isempty(jobid)
  warning(defaultId, 'FieldTrip:peer:noSlaveAvailable', 'none of the slave peers was willing to accept the job');
end
