...
for i=1:length(varargin)
  ...
  if isfield(varargin{i},'trial') && isfield(varargin{i},'avg');% see bug2372 (dieloz)
    ...
    warning(defaultId, 'depreciating trial field: using the avg to compute the grand average');
    if strcmp(varargin{i}.dimord,'rpt_chan_time');
      varargin{i}.dimord = 'chan_time';
    end
  else
    if isfield(varargin{i},'trial') && ~isfield(varargin{i},'avg');
      error(defaultId, 'input dataset %d does not contain avg field: see ft_timelockanalysis', i);
    end
  end
end
...
if strcmp(cfg.parameter,'trial');
  error(defaultId, 'not supporting averaging over the repetition dimension');
end
...
if strcmp(cfg.keepindividual, 'yes')
  ...
  for s=1:Nsubj
    switch cfg.method
      ...
      otherwise
        error(defaultId, 'unsupported value for cfg.method')
    end % switch
  end
