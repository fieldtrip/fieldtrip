  ...
  case {'ctf64', 'ctf151', 'ctf275' 'bti148', 'bti248', 'bti248grad', 'itab28', 'itab153', 'yokogawa64', 'yokogawa160', 'babysquid74'}
    ...
    if all(numcoils==numcoils(1))
      ...
      while ~all(numcoils==tmp)
        ...
        numcoils = sum(isfinite(dist),2);
        if niter>500
          error(defaultId, 'Failed to extract the positions of the channels. This is most likely due to the balancing matrix being rank deficient. Please replace data.grad with the original grad-structure obtained after reading the header.');
        end
      end
    else
...
if length(lab)~=nchan || size(pnt,1)~=nchan || size(ori,1)~=nchan
  warning(defaultId, 'the positions were not determined for all channels');
end
