...
if doinit,
    ...
    lat_in = lat_est;
    for m = 1:ncomp
        [lat_tmp, amp_tmp] = estimate_parameters(cfg, data, erp_est, amp_in, lat_in, m, noise);
        lat_est(:,m) = lat_tmp(:,m);
        amp_est(:,m) = amp_tmp(:,m);
    end
end
...
for k = 1:numiteration
    %reject trials
    [rejectflag, corr_est] = rejecttrial(cfg, erp_est, amp_est, lat_est, data);
    rejectflag(:) = false;
    
    % ERP estimation in frequency domain
    [erp_est, amp_est, lat_est, residual_f] = estimate_erp(cfg, data, erp_est, amp_est, lat_est, noise, rejectflag);
    ...
    if strcmp(cfg.aseo.noiseEstimate, 'parametric')
        ...
        [noise, ar, sigma] = estimate_ar(residual, rejectflag, Nsmp, maxOrderAR);
        noise              = noise(1:(Nsmp/2+1),:);
        
        % also compute the AR model for the original data
        [orig, ar_orig, sigma_orig] = estimate_ar(temp2, rejectflag, Nsmp, maxOrderAR);
        orig  = orig(1:(Nsmp/2+1),:);
        
        % also compute the AR model for the ERP subtracted data
        [noise3, ar_orig2, sigma_orig2] = estimate_ar(temp3, rejectflag, Nsmp, maxOrderAR);
...
function [lat_est, amp_est] = estimate_parameters(cfg, data, erp_est, amp_est, lat_est, compNo, noise)
...
function [rejectflag, corr_est] = rejecttrial(cfg, erp_est, amp_est, lat_est, data)
...
function [erp_est, amp_est, lat_est, residual_f] = estimate_erp(cfg, data, erp_in, amp_in, lat_in, noise, rejectflag)
...
amp_est = amp_in;
for compNo = 1:ncomp
    [tmp_lat, tmp_amp] = estimate_parameters(cfg, data, erp_est, amp_in, lat_in, compNo, noise);
...
function  [power, coeffAR, sigma] = estimate_ar(dat, rejectflag, nfft, maxOrderAR)
...
function [orig, reconstructed] = reconstruct_erp(input, avgflag)
