...
if ~(exist('designtype')==1)
    warning(defaultId, 'Unknown test statistic.');
    return
end

if ~isfield(cfg,'design')
  error(defaultId, 'You should specify an initial design in cfg.design.');
end
...
if strcmp(designtype,'between')   % between-units conditions
  ...
  if isfield(cfg,'ext')
    if size(cfg.ext,1)~=size(initialdesign,1)
        error(defaultId, 'Incompatible number of replications in cfg.ext.');
    end
    design=cfg.ext;
  end
...
elseif strcmp(designtype,'within')  % within-units conditions
  ...
  for wcondindx=1:nwcond
    ...
    if length(unitthiscond)==length(unitlabels) && ~all(unitthiscond==unitlabels)
      error(defaultId, 'The last two columns of initialdesign do not specify a within-units design.');
    end
  end
  if ~isfield(cfg,'ivar')
    cfg.ivar = 1;
  ...
  if isfield(cfg,'ext')
    ...
    elseif dimext(1)==nrepl
      ...
      error(defaultId, 'The number of rows in cfg.ext must be equal to the number of conditions or the number of replications (number of conditions times number of units-of-observation).');
    end
