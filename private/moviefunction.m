...
for i=1:numel(varargin)
  if ~isempty(cfg.yparam) && ~isfield(varargin{i}, 'freq')
    error(defaultId, 'data argument %i does not have a .freq field, while all former had', i);
...
for i = 1:Ndata
  ...
  if opt.issource(i)+opt.isfreq(i)+opt.istimelock(i) ~= 1
    error(defaultId, 'data argument %i cannot be definitely identified as frequency, timelock or source data', i);
  end
end
...
if Ndata>1,
  if all(opt.issource==1),
    ...
    error(defaultId, 'currently, not more than 1 data argument is allowed, unless one of them is a parcellation, and the other a channel level structure');
  ...
  elseif Ndata==2 && sum(opt.issource==1)
    ...
    if ~ft_datatype(varargin{opt.issource}, 'parcellation')
      error(defaultId, 'the source data structure should be a parcellation');
    end
  end
end
...
if any(strcmp(dimtok, 'rpt') | strcmp(dimtok, 'subj'))
  error(defaultId, 'the input data cannot contain trials or subjects, please average first using ft_selectdata');
end
...
if opt.zdim ~=1
  error(defaultId, 'input %i data does not have the correct format of N x time (x freq)', i);
end
...
%     end
%   else
%     error(defaultId, 'Either moviefreq or movietime should contain a bin number')
%   end
% else
%   for iFrame = 1:floor(size(opt.dat, opt.xdim)/cfg.samperframe)
%     indx = ((iFrame-1)*cfg.samperframe+1):iFrame*cfg.samperframe;
