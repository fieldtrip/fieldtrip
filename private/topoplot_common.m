...
if isnumeric(cfg.highlight)
  ...
  cfg.highlight = 'on';
  warning(defaultId, 'cfg.highlight is now used for specifying highlighting-mode, use cfg.highlightchannel instead of cfg.highlight for specifying channels')
...
if strcmp(cfg.marker,'highlights')
  warning(defaultId, 'using cfg.marker option -highlights- is no longer used, please use cfg.highlight')
  cfg.marker = 'off';
end

% check colormap is proper format and set it
if isfield(cfg,'colormap')
  if size(cfg.colormap,2)~=3, error(defaultId, 'topoplot(): Colormap must be a n x 3 matrix'); end
...
switch dtype
  ...
  otherwise
    ...
    if ~isfield(data, 'label'), error(defaultId, 'the input data should at least contain a label-field'); end
    if ~isfield(cfg, 'parameter'), error(defaultId, 'the configuration should at least contain a ''parameter'' field'); end
...
if isfield(cfg, 'parameter') && ~isfield(data, cfg.parameter)
  error(defaultId, 'cfg.parameter=%s is not present in data structure', cfg.parameter);
end
...
if ~isempty(cfg.parameter)
  ...
  error(defaultId, 'cannot make selection of data');
end
...
if isfield(data, cfg.maskparameter)
  ...
  if size(msk,2)>1 || size(msk,3)>1
    warning(defaultId, 'no masking possible for average over multiple latencies or frequencies -> cfg.maskparameter cleared')
    msk = [];
  end
...
elseif ~ischar(cfg.comment)
  error(defaultId, 'cfg.comment must be string');
end
...
if strcmp(cfg.interpolatenan,'yes') && any(nanInds)
  warning(defaultId, 'removing NaNs from the data');
  chanX(nanInds) = [];
  chanY(nanInds) = [];
  dat(nanInds)   = [];
  if ~isempty(msk)
...
if strcmp(cfg.interactive, 'yes')
  ...
  elseif any(strcmp(data.dimord, {'chan_freq_time', 'subj_chan_freq_time', 'rpt_chan_freq_time', 'rpttap_chan_freq_time', 'chan_chan_freq_time', 'chancmb_freq_time', 'rpt_chancmb_freq_time', 'subj_chancmb_freq_time'}))
    ...
    warning(defaultId, 'unsupported dimord "%s" for interactive plotting', data.dimord);
  end
end
