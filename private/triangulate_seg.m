...
if ~any(seg(:))
  error(defaultId, 'the segmentation is empty')
end
...
if num>1,
  warning(defaultId, 'the segmented volume consists of more than one compartment, using only the biggest one for the segmentation');
...
if ishollow
  ...
  warning(defaultId, 'the segmentation is not star-shaped, please check the surface mesh');
end
