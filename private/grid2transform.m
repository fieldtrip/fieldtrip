...
if any(size(oldtransform)~=[4 4])
  error(defaultId, 'incorrect size of homogenous transformation matrix');
elseif ~all(abs(diff(xgrid,2))<=100*eps*max(abs(xgrid)))
  error(defaultId, 'xgrid is not monotonously decreasing or increasing');
elseif ~all(abs(diff(ygrid,2))<=100*eps*max(abs(ygrid)))
  error(defaultId, 'ygrid is not monotonously decreasing or increasing');
elseif ~all(abs(diff(zgrid,2))<=100*eps*max(abs(zgrid)))
  error(defaultId, 'zgrid is not monotonously decreasing or increasing');
end
...
if any(volume.dim(1:3)~=[Nx Ny Nz])
  error(defaultId, 'dimensions do not correspond');
end
