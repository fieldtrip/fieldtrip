...
if ~exist('avw','var')
    doc avw_img_write;
    error(defaultId, '...no input avw.');
elseif isempty(avw)
    error(defaultId, '...empty input avw.');
elseif ~isfield(avw,'img')
    error(defaultId, '...empty input avw.img');
end
...
if isempty(fileprefix)
    ...
    if ~fileprefix
        doc avw_img_write;
        error(defaultId, 'no output .hdr file specified');
    end
end
...
if fid < 0
    error(defaultId, 'Cannot open file %s.img\n',fileprefix);
else
...
if ~exist('avw','var')
    warning(defaultId, '...no input avw - calling avw_hdr_make\n');
    avw = avw_hdr_make;
elseif isempty(avw)
    warning(defaultId, '...empty input avw - calling avw_hdr_make\n');
    avw = avw_hdr_make;
elseif ~isfield(avw,'hdr')
    warning(defaultId, '...empty input avw.hdr - calling avw_hdr_make\n');
    avw = avw_hdr_make;
end
if ~isequal(avw.hdr.hk.sizeof_hdr,348)
    error(defaultId, '...avw.hdr.hk.sizeof_hdr must be 348!\n');
end
...
if ~exist('fileprefix','var')
    ...
    if quit
        ...
        error(defaultId, '...no input fileprefix - see help avw_hdr_write\n\n');
        return;
    end
end
...
if fid < 0
    error(defaultId, 'Cannot write to file %s.hdr\n',fileprefix);
else
...
case  32
    error(defaultId, '...complex datatype not yet supported.\n');
case  64
    avw.hdr.dime.bitpix = int16(64); precision = 'double';
case 128
    error(defaultId, '...RGB datatype not yet supported.\n');
otherwise
    warning(defaultId, '...unknown datatype, using type 16 (32 bit floats).\n');
