...
if tmp>1
  error(defaultId, 'only one resampling strategy should be specified');
elseif tmp<1
  ...
  for c=1:Ncondition
    ...
    for d=1:Ndata
      ...
      elseif siz(1)==1
        ...
        error(defaultId, 'inconsistent number of replications in data');
      end
    end 
  end 
...
elseif resamplejackknife
  if Ncondition>1
    error(defaultId, 'jackknife requires exactly one condition');
  end
  % the number of replications of all dataobjects should be the same 
...
elseif resamplebootstrap
  if Ncondition>1
    error(defaultId, 'bootstrap requires exactly one condition');
  end
  % the number of replications of all dataobjects should be the same 
  Nreplication = max(Nreplication(:));
...
elseif resamplepseudovalue
  if Ncondition>1
    error(defaultId, 'pseudovalue requires exactly one condition');
  end
  % the number of replications of all dataobjects should be the same 
  Nreplication = max(Nreplication(:));
  ...
  for d=1:Ndata
    ...
    elseif siz(1)==1
      error(defaultId, 'multiple replications are required in data');
    else
      error(defaultId, 'inconsistent number of replications in data');
    end
  end 
...
elseif resamplerandomization
  if Ncondition<2
    error(defaultId, 'randomization requires at least two conditions');
  end
  ...
  for d=1:Ndata
    ...
    for c=2:Ncondition
      if ~all(siz(c, 2:end)==siz(1, 2:end))
        error(defaultId, 'the dimensions should be the same for every replication in every condition')
      end
    end
...
elseif resamplepermutation
  if Ncondition<2
    error(defaultId, 'permutation requires at least two conditions');
  end
  ...
  if Ncondition<=2
    ...
    if ischar(cfg.numpermutation) && strcmp(cfg.numpermutation, 'all')
      ...
      if cfg.numpermutation>2^Nreplication
        error(defaultId, 'it is not possible to make more random permutations than %d', 2^Nreplication);
      end
      number = [];
    ...
    if ischar(cfg.numpermutation) && strcmp(cfg.numpermutation, 'all')
      error(defaultId, 'it is not possible to generate all possible permutations for more than two conditions');
    end
  ...
  for d=1:Ndata
    ...
    for c=2:Ncondition
      if ~all(siz==size(datain{c,d}))
        error(defaultId, 'the dimensions should be the same for every condition')
      end
    end
    % concatenate the data in all conditions to facilitate the random resamplings
    dataconcat = cat(1, datain{:,d});
  ...
  error(defaultId, 'unknown resampling strategy')
end
