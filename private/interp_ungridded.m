...
if nargin<3
  error(defaultId, 'Not enough input arguments.');
end
  ...
  switch projmethod
    case 'nearest'
      if ~isempty(sphereradius)
        warning(defaultId, 'sphereradius is not used for projmethod ''nearest''');
      end
    ...
    case {'sphere_avg', 'sphere_weighteddistance'}
      if isempty(sphereradius)
        error(defaultId, 'sphereradius should be specified');
      end
    ...
    case 'smudge'
      if isempty(triout),
        error(defaultId, 'the ''smudge'' method needs a triangle definition');
      end
    ...
    otherwise
      error(defaultId, 'unsupported projection method');
  end % case projmethod
