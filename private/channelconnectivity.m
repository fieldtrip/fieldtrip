...
if (isfield(cfg, 'avgoverchan') && strcmp(cfg.avgoverchan, 'yes')) || isempty(cfg.neighbours)
  ...
  elseif nargin == 2
    ...
    error(defaultId, 'either the cfg needs to have both cfg.channel and cfg.neighbours, or a second (data) input argument needs to be specified');
  end
  ...
  try
    ...
    if numel(neighbours) < nchan/4
      error(defaultId, 'channelconnectivity only works when at least 1/4th of all channels has neighbours defined (or with neighbours = [])');
