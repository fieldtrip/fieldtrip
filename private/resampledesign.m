...
if ~isempty(intersect(cfg.ivar, cfg.uvar)), warning(defaultId, 'there is an intersection between cfg.ivar and cfg.uvar'); end
if ~isempty(intersect(cfg.ivar, cfg.wvar)), warning(defaultId, 'there is an intersection between cfg.ivar and cfg.wvar'); end
if ~isempty(intersect(cfg.ivar, cfg.cvar)), warning(defaultId, 'there is an intersection between cfg.ivar and cfg.cvar'); end
if ~isempty(intersect(cfg.uvar, cfg.wvar)), warning(defaultId, 'there is an intersection between cfg.uvar and cfg.wvar'); end
if ~isempty(intersect(cfg.uvar, cfg.cvar)), warning(defaultId, 'there is an intersection between cfg.uvar and cfg.cvar'); end
if ~isempty(intersect(cfg.wvar, cfg.cvar)), warning(defaultId, 'there is an intersection between cfg.wvar and cfg.cvar'); end
...
if ~isempty(cfg.wvar)
  ...
  if any(blklen~=blklen(1))
    error(defaultId, 'the number of repetitions per block should be constant');
  end
  for i=1:size(blkmeas,2)
    if any(diff(design(:, blksel{i}), 1, 2)~=0)
      error(defaultId, 'the design matrix variables should be constant within a block');
    end
  end
  orig_design = design;
  orig_Nrepl  = Nrepl;
...
if Nvar==1 && ~isempty(cfg.uvar)
  error(defaultId, 'A within-units shuffling requires a at least one unit variable and at least one independent variable');
end
...
elseif ~isempty(cfg.uvar) && strcmp(cfg.resampling, 'permutation')
  ...
  if ischar(cfg.numrandomization) && strcmp(cfg.numrandomization, 'all')
    ...
    if any(unitlen~=2)
      error(defaultId, 'cfg.numrandomization=''all'' is only supported for two repeated measurements');
    end
...
elseif length(cfg.uvar)==1 && strcmp(cfg.resampling, 'bootstrap') && isempty(cfg.cvar),
  ...
  if any(Nrep~=Nrep(1)), error(defaultId, 'all units of observation should have an equal number of repetitions'); end
  
  if max(units(:))<20,
    warning(defaultId, 'fewer than 20 units warrants explicit checking of double occurrences of ''bootstraps''');
    checkunique = 1;
  else
    checkunique = 0;
  end
  ...
  error(defaultId, 'Unsupported configuration for resampling.');
end
...
if strcmp(efficient, 'yes')
  if numel(cfg.ivar)<1
    error(defaultId, 'this reqiures at least one independent variable to be specified (ivar)');
  end
  if numel(cfg.uvar)>0
    error(defaultId, 'this is not yet supported in combination with a unit of observation (uvar)');
  end
