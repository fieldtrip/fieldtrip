...
if cfg.tail~=cfg.clustertail
  error(defaultId, 'cfg.tail and cfg.clustertail should be identical')
end
...
if strcmp(cfg.clusterthreshold, 'parametric')
  ...
  elseif prod(siz)==2 && cfg.clustertail==0
    ...
    error(defaultId, 'cannot make sense out of the specified parametric critical values');
  end
...
elseif strcmp(cfg.clusterthreshold, 'nonparametric_common')
  ...
  error(defaultId, 'no valid threshold for clustering was given')
...
if (Nobspos+Nobsneg)==0
  warning(defaultId, 'no clusters were found in the observed data');
  ...
  if needpos,
    ...
    for j = 1:Nrndpos
      ...
      elseif strcmp(cfg.clusterstatistic, 'wcm')
        ...
        error(defaultId, 'unknown clusterstatistic');
      end
    end % for 1:Nrdnpos
    if strcmp(cfg.multivariate, 'yes') || strcmp(cfg.orderedstats, 'yes')
