...
if isfield(cfg,'reject')
  warning(defaultId, 'converting from old-style artifact configuration to new-style');
...
if ~isempty(cfg.artfctdef.crittoilim)
  ...
  if (size(cfg.artfctdef.crittoilim,2) ~= 2 ...
    ...
    error(defaultId, 'if specified, cfg.artfctdef.crittoilim should be a 1x2 or Nx2 vector');
  end
...
if isempty(trl)
  error(defaultId, 'no trials were selected, cannot perform artifact detection/rejection');
end
...
if strcmp(cfg.artfctdef.reject, 'nan') && ~hasdata
    error(defaultId, 'If bad parts are to be filled with nans, input data has to be specified');
end;
...
if isfield(cfg.artfctdef, 'writerej') && ~isempty(cfg.artfctdef.writerej)
  ...
  if fid<0
    error(defaultId, 'could not open rejection file for writing');
  else
...
if isempty(cfg.trl)
  error(defaultId, 'No trials left after artifact rejection.')
else
