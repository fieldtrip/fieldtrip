...
if isfield(cfg.grid, 'resolution') && isfield(cfg.grid, 'xgrid') && ~ischar(cfg.grid.xgrid)
  error(defaultId, 'You cannot specify cfg.grid.resolution and an explicit cfg.grid.xgrid simultaneously');
end
if isfield(cfg.grid, 'resolution') && isfield(cfg.grid, 'ygrid') && ~ischar(cfg.grid.ygrid)
  error(defaultId, 'You cannot specify cfg.grid.resolution and an explicit cfg.grid.ygrid simultaneously');
end
if isfield(cfg.grid, 'resolution') && isfield(cfg.grid, 'zgrid') && ~ischar(cfg.grid.zgrid)
  error(defaultId, 'You cannot specify cfg.grid.resolution and an explicit cfg.grid.zgrid simultaneously');
end
...
if sum([basedonresolution basedongrid basedonpos basedonshape basedonmri basedonvol basedoncortex basedonmni])~=1
  error(defaultId, 'incorrect cfg specification for constructing a dipole grid');
end
...
if strcmp(cfg.grid.unit, 'auto')
  ...
  elseif ~isempty(headmodel)
    ...
    warning(defaultId, 'assuming "cm" as default source units');
    cfg.grid.unit = 'cm';
  end
end
...
if basedonresolution
  ...
  elseif ~isempty(headmodel)
    ...
    error(defaultId, 'creating a 3D grid based on resolution requires either sensor positions or a headmodel to estimate the extent');
  end
...
if basedonmri
  ...
  elseif ft_datatype(mri, 'segmentation')
    ...
    error(defaultId, 'cannot determine the format of the segmentation in cfg.mri');
  end
...
if basedonshape
  ...
  elseif ischar(cfg.headshape)
    ...
    error(defaultId, 'cfg.headshape is not specified correctly')
  end
  % ensure that the headshape is in the same units as the source
...
if basedonmni
  if ~isfield(cfg.grid, 'template') && ~isfield(cfg.grid, 'resolution')
    error(defaultId, 'you either need to specify the filename of a template grid in cfg.grid.template, or a resolution in cfg.grid.resolution');
  ...
  if ischar(cfg.mri)
    if ~exist(fname, 'file')
      error(defaultId, 'the MNI template grid based on the specified resolution does not exist');
    end
    mri = ft_read_mri(cfg.mri);
  else
    mri = cfg.mri;
...
if strcmp(cfg.spherify, 'yes')
  if ~ft_voltype(headmodel, 'singlesphere') && ~ft_voltype(headmodel, 'concentricspheres')
    error(defaultId, 'this only works for spherical volume conduction models');
  end
...
if ~isempty(cfg.symmetry)
  if size(grid.pos,2)>3
    ...
    warning(defaultId, 'the construction of a symmetric dipole model requires to start with a Nx3 description of the dipole positions, discarding subsequent columns');
  ...
  elseif strcmp(cfg.symmetry, 'z')
    ...
    error(defaultId, 'unrecognized symmetry constraint');
  end
