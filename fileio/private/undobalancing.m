...
while ~isempty(sens.balance.current)
  this_name    = sens.balance.current{end};
  this_montage = ft_inverse_montage(sens.balance.(this_name));
  fprintf('reverting the "%s" projection\n', this_name);
  sens = ft_apply_montage(sens, this_montage, 'keepunused', 'no');
  sens.balance.current = sens.balance.current(1:end-1); % remove this from the list
...
% while isfield(sens, 'balance') && isfield(sens.balance, 'current') && ~isempty(sens.balance.current)
