  ...
  switch seldim
    case 'rpt'
      ...
      elseif strcmp(data.(fld)(1:4), 'rpt_')
        ...
        if ~all(n==n(1))
          error(defaultId, 'inconsistent number of repetitions for dim "%s"', seldim);
        end
        n = n(1);
    ...
    case 'subj'
      ...
      if ~all(n==n(1)), error(defaultId, 'inconsistent number of repetitions for dim "%s"', seldim); end
      n = n(1);
    ...
    otherwise
      error(defaultId, 'unsupported dim "%s"', seldim);
  end
