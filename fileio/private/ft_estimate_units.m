...
if indx>length(unit)
  indx = length(unit);
  warning(defaultId, 'assuming that the units are "%s"', unit{indx});
elseif indx<1
  indx = 1;
  warning(defaultId, 'assuming that the units are "%s"', unit{indx});
elseif abs((est-floor(est)) - 0.5)<0.1
  ...
  warning(defaultId, 'the estimated units are not very decisive, assuming that the units are "%s"', unit{indx});
end
