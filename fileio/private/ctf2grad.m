...
elseif isfield(hdr, 'res4') && isfield(hdr.res4, 'senres')
  ...
  if numEEG>0
    for i=1:numEEG
      ...
      if hdr.res4.senres(n).numCoils~=1
        error(defaultId, 'unexpected number of electrode positions in EEG channel');
      end
      % add this position
  ...
  for i=1:numMEG
    ...
    if hdr.res4.senres(n).numCoils~=2
      error(defaultId, 'unexpected number of coils in MEG channel');
    end
  ...
  elseif all([hdr.res4.senres(selMEG).grad_order_no]==13)
    ...
    warning(defaultId, 'cannot determine balancing of CTF gradiometers');
...
elseif isfield(hdr, 'sensor') && isfield(hdr.sensor, 'info')
  ...
  if dewar
    ...
    error(defaultId, 'cannot return the gradiometer definition in dewar coordinates');
  end
  ...
  for i=1:length(sel)
    ...
    elseif size(ori,1)==1 && size(pnt,1)==2
      ...
      error(defaultId, 'do not know how to deal with higher order gradiometer hardware')
    end
  ...
  error(defaultId, 'unknown header to contruct gradiometer definition');
end
