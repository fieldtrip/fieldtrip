...
if ~isempty(format)
  warning(defaultId, 'the option ''format'' will be deprecated soon, please use ''dataformat'' instead');
...
if ~exist(filename, 'file')
  error(defaultId, 'file ''%s'' does not exist', filename);
end
...
case 'analyze_old'
  ...
  warning(defaultId, 'flipping 1st dimension (L-R) to obtain volume in neurological convention');
  img = flipdim(img, 1);
...
case {'afni_brik' 'afni_head'}
  ...
  if err
    error(defaultId, 'could not read AFNI file');
  end
  ...
  warning(defaultId, 'homogenous transformation might be incorrect for AFNI file');
  transform        = eye(4);
...
case 'neuromag_fif'
  ...
  if isfield(hdr, 'trans') && issubfield(hdr.trans, 'trans')
    if (hdr.trans.from == 4) && (hdr.trans.to == 5)
      ...
      warning(defaultId, 'W: trans does not transform from 4 to 5.');
      warning(defaultId, 'W: Please check the MRI fif-file');
    end
  else
    warning(defaultId, 'W: trans structure is not defined.');
    warning(defaultId, 'W: Maybe coregistration is missing?');
  end
  if isfield(hdr, 'voxel_trans') && issubfield(hdr.voxel_trans, 'trans')
    ...
    if (hdr.voxel_trans.from == 2001) && (hdr.voxel_trans.to == 5)
      ...
      warning(defaultId, 'W: voxel_trans does not transform from 2001 to 5.');
      warning(defaultId, 'W: Please check the MRI fif-file');
    end
  else
    warning(defaultId, 'W: voxel_trans structure is not defined.');
    warning(defaultId, 'W: Please check the MRI fif-file');
  end
...
case 'dicom_old'
  ...
  if isempty(dirlist)
    ...
    warning(defaultId, 'could not determine list of dicom files, trying with *.dcm');
    dirlist  = dir(fullfile(p, '*.dcm'));
    dirlist  = {dirlist.name};
  end
  
  if isempty(dirlist)
    warning(defaultId, 'could not determine list of dicom files, trying with *.ima');
...
case {'nifti', 'freesurfer_mgz', 'freesurfer_mgh', 'nifti_fsl'}
  if strcmp(dataformat, 'freesurfer_mgz') && ispc
    error(defaultId, 'Compressed .mgz files cannot be read on a PC');
  end
...
case 'yokogawa_mri'
  ...
  error(defaultId, 'FIXME yokogawa_mri implementation is incomplete');

case 'matlab'
  mri = loadvar(filename, 'mri');

otherwise
  error(defaultId, 'unrecognized filetype ''%s'' for ''%s''', dataformat, filename);
end
