...
if iscell(filename)
  warning(defaultId, 'concatenating data from %d files', numel(filename));
...
if ~isempty(begsample) && mod(begsample, 1)
  warning(defaultId, 'rounding "begsample" to the nearest integer');
  begsample = round(begsample);
end
if ~isempty(endsample) && ~isinf(endsample) && mod(endsample, 1)
  warning(defaultId, 'rounding "endsample" to the nearest integer');
  endsample = round(endsample);
end
if ~isempty(begtrial) && mod(begtrial, 1)
  warning(defaultId, 'rounding "begtrial" to the nearest integer');
  begtrial = round(begtrial);
end
if ~isempty(endtrial) && mod(endtrial, 1)
  warning(defaultId, 'rounding "endtrial" to the nearest integer');
...
if any(begsample<1)
  error(defaultId, 'FILEIO:InvalidBegSample', 'cannot read data before the begin of the file');
elseif any(endsample>(hdr.nSamples*hdr.nTrials)) && ~blocking
  error(defaultId, 'FILEIO:InvalidEndSample', 'cannot read data after the end of the file');
end
...
if cache && requesttrials
  error(defaultId, 'caching is not supported when reading trials')
end
...
if requesttrials && requestsamples
  error(defaultId, 'you cannot select both trials and samples at the same time');
...
elseif requestsamples
  ...
  error(defaultId, 'you should either specify begin/end trial or begin/end sample');
end
...
if checkboundary && hdr.nTrials>1
  if begtrial~=endtrial
    error(defaultId, 'requested data segment extends over a discontinuous trial boundary');
  end
end
...
switch dataformat
  ...
  case {'4d' '4d_pdf', '4d_m4d', '4d_xyz'}
    ...
    if isfield(hdr.orig, 'ChannelUnitsPerBit')
      ...
      warning(defaultId, 'cannot determine ChannelUnitsPerBit');
      upb = 1;
    end
  ...
  case 'biosemi_old'
    ...
    for i=begepoch:endepoch
      ...
      if size(buf,2)~=hdr.nChans || size(buf,1)~=epochlength
        error(defaultId, 'error reading selected data from bdf-file');
      else
  ...
  case {'egi_mff_v1' 'egi_mff'} % this is currently the default
    ...
    if isfield(hdr.orig.xml,'epochs') && length(hdr.orig.xml.epochs) > 1
      if hdr.nTrials ==1
        ...
        if sum(data_in_epoch>1) > 1
          warning(defaultId, 'The requested segment from %i to %i is spread out over multiple epochs with possibly discontinuous boundaries', begsample, endsample);
        end
      end
    end
    ...
    if isempty(binfiles)
      error(defaultId, 'FieldTrip:read_mff_header:nobin', ['could not find any signal.bin in ' filename_mff ])
    end
  ...
  case 'fcdc_buffer'
    ...
    if blocking
      ...
      if available.nsamples<nsamples
        error(defaultId, 'buffer timed out while waiting for %d samples', nsamples);
      end
    end
  ...
  case 'fcdc_mysql'
    ...
    db_open(filename);
    if db_blob
      error(defaultId, 'not implemented');
    else
  ...
  case 'itab_raw'
    ...
    elseif any(hdr.orig.data_type==[3 4 5])
      ...
      error(defaultId, 'unsuppported data_type in itab format');
    end
  ...
  case 'neuroscope_bin'
    switch hdr.orig.nBits
      ...
      case 32
        precision = 'int32';
      otherwise
        error(defaultId, 'unknown precision');
    end
  ...
  case {'ns_cnt' 'ns_cnt16', 'ns_cnt32'}
    ...
    if sample1<0
      error(defaultId, 'begin sample cannot be for the beginning of the file');
    end
  ...
  case 'neuromag_mex'
    ...
    for i=begepoch:endepoch
      ...
      if ~strcmp(status, 'ok')
        error(defaultId, 'error reading selected data from fif-file');
      else
  ...
  case {'neurosim_ds' 'neurosim_signals'}
    ...
    if endsample>size(dat,2)
      warning(defaultId, 'Simulation was not completed, reading in part of the data')
  ...
  case 'neurosim_spikes'
    warning(defaultId, 'Reading Neurosim spikes as continuous data, for better memory efficiency use spike structure provided by ft_read_spike instead.');
    ...
    warning(defaultId, 'off','FieldTrip:ft_read_event:unsupported_event_format')
    cfg = ft_definetrial(cfg);
    warning(defaultId, 'on','FieldTrip:ft_read_event:unsupported_event_format')
  ...
  case {'plexon_nex' 'read_plexon_nex'} % this is the default reader for nex files
    ...
    if any(isnan(dat(:)))
      warning(defaultId, 'data has been padded with NaNs');
    end
  ...
  case 'plexon_plx'
    ...
    if (length(contindx)+length(spikeindx))<length(chanindx)
      error(defaultId, 'not all selected channels could be located in the data');
    end
  ...
  otherwise
    ...
    try
      ...
      if strcmp(fallback, 'biosig') && ft_hastoolbox('BIOSIG', 1)
        ...
        error(defaultId, 'unsupported data format (%s)', dataformat);
      end
    end
...
switch dimord
  ...
  case 'trials_chans_samples'
    ...
    dimord = 'chans_samples_trials';
  otherwise
    error(defaultId, 'unexpected dimord');
end
...
if ~isempty(chanunit)
  if length(chanunit)~=length(chanindx)
    error(defaultId, 'the number of channel units is inconsistent with the number of channels');
  end
