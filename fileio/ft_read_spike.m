...
if ~exist(filename,'file')
    error(defaultId, 'File or directory does not exist')
end
...
switch spikeformat
  ...
  case {'neuralynx_ncs' 'plexon_ddt'}
    ...
    error(defaultId, 'file does not contain spike timestamps or waveforms');
  ...
  case 'neuroscope'
    ...
    for k = 1:numel(spike.label)
      ...
      if any(abs(timestamps-round(timestamps))>1e-5),
        error(defaultId, 'there seems to be a mismatch between the spike times and the expected integer-valued timestamps');
      end
  ...
  otherwise
    error(defaultId, ['unsupported data format (' spikeformat ')']);
end
