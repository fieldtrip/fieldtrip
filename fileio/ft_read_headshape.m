...
if iscell(filename)
  ...
  if  numel(filename)>1 && all(haspos==1) && strcmp(concatenate, 'yes')
    if length(bnd)>2
      error(defaultId, 'Cannot concatenate more than two files') % no more than two files are taken for cancatenation
    else
      ...
      elseif ~isfield(bnd(1), 'tri') && ~isfield(bnd(2), 'tri')
        ...
        error(defaultId, 'not all input files seem to contain a triangulation');
      end
      ...
      for k = 1:numel(fnames)
        ...
        elseif ~isfield(bnd(1), fnames{k}) && ~isfield(bnd(2), fnames{k})
          ...
          error(defaultId, 'not all input files seem to contain a "%s"', fnames{k});
        end
      end
  ...
  elseif numel(filename)>1 && ~all(haspos==1)
    if numel(bnd)>2
      error(defaultId, 'Cannot combine more than two files') % no more than two files are taken for cancatenation
    else
      shape = [];
      if sum(haspos==1)==1
        ...
        error(defaultId, 'Don''t know what to do');
      end
      if sum(hastri==1)==1
      ...
      if max(shape.tri(:))~=size(shape.pos,1)
        error(defaultId, 'mismatch in number of points in pos and tri');
      end
    end
...
if ~isempty(annotationfile) && ~strcmp(fileformat, 'mne_source')
  error(defaultId, 'at present extracting annotation information only works in conjunction with mne_source files');
end
...
switch fileformat
  case {'ctf_ds', 'ctf_hc', 'ctf_meg4', 'ctf_res4', 'ctf_old'}
    ...
    switch coordsys
      ...
      otherwise
        error(defaultId, 'incorrect coordsys specified');
    end
  ...
  case 'gifti'
    ...
    if ~isfield(g, 'vertices')
      error(defaultId, '%s does not contain a tesselated surface', filename);
    end
  ...
  case 'caret_spec'
    ...
    if max(tmp2.tri(:))~=size(tmp1.pos,1)
      error(defaultId, 'there''s a mismatch between the number of points used in the topology, and described by the coordinates');
    end
  ...
  case 'mne_source'
    ...
    if ~isempty(annotationfile)
      ...
      if numel(annotationfile)~=2
        error(defaultId, 'two annotationfiles expected, one for each hemisphere');
      end
      ...
      elseif src(1).np == numel(label{2}) && src(1).np == numel(label{1})
        ...
        warning(defaultId, 'incompatible annotation with triangulations, not using annotation information');
      end
      if ~isequal(c(1),c(2))
        error(defaultId, 'the annotation tables differ, expecting equal tables for the hemispheres');
      end
      c = c(1);
    end
  ...
  case {'neuromag_fif' 'neuromag_mne'}
    ...
    switch coordsys
      ...
      otherwise
        error(defaultId, 'incorrect coordinates specified');
    end
  ...
  case {'yokogawa_mrk', 'yokogawa_ave', 'yokogawa_con', 'yokogawa_raw' }
    ...
    if any([marker(:).meg_pos])
      ...
      error(defaultId, 'no coil information found in Yokogawa file');
    end
  ...
  case 'yokogawa_coregis'
    ...
    if size(shape.fid.label,1) ~= 5
      error(defaultId, 'Wrong number of coils');
    end
  ...
  case 'spmeeg_mat'
    ...
    if isfield(tmp.D, 'fiducials') && ~isempty(tmp.D.fiducials)
      ...
      error(defaultId, 'no headshape found in SPM EEG file');
    end
  ...
  case 'matlab'
    ...
    elseif isfield(tmp, 'Vertices')
      ...
      error(defaultId, 'no headshape found in MATLAB file');
    end
  ...
  case 'netmeg'
    ...
    if isfield(hdr.orig, 'headshapedata')
      ...
      error(defaultId, 'the NetMEG file "%s" does not contain headshape data', filename);
    end
  ...
  case 'vista'
    ...
    elseif size(elements,2)==4
      ...
      error(defaultId, 'unknown elements format')
    end
  ...
  case 'brainsuite_dfs'
    ...
    elseif exist(fullfile(p, [f '.nii.gz']), 'file')
      ...
      warning(defaultId, 'could not find accompanying MRI file, returning vertices in voxel coordinates');
    end
  ...
  case 'brainvisa_mesh'
    ...
    if isempty(transform)
      warning(defaultId, 'cound not determine the coordinate transformation, returning vertices in voxel coordinates');
    end
  ...
  otherwise
    ...
    if ~success
      ...
      try
        ...
        if ~ft_senstype(elec, 'eeg')
          error(defaultId, 'headshape information can not be read from MEG gradiometer file');
        else
          shape.fid.pos   = elec.chanpos;
          shape.fid.label = elec.label;
          success = 1;
    ...
    if ~success
      ...
      try
        ...
        if ~ft_voltype(headmodel, 'bem')
          error(defaultId, 'skin surface can only be extracted from boundary element model');
        else
    ...
    if ~success
      error(defaultId, 'unknown fileformat "%s" for head shape information', fileformat);
    end
