...
switch dataformat
    ...
    if ~append && ~isempty(hdr)
      ...
      try
        ...
        if ~isempty(strfind(lasterr, 'Buffer size N must be an integer-valued scalar double.'))
          ...
          error(defaultId, 'the FieldTrip buffer mex file was not found on your path, it should be in fieldtrip/fileio/private');
        ...
        elseif ~isempty(strfind(lasterr, 'failed to create socket')) && (strcmp(host, 'localhost') || strcmp(host, '127.0.0.1'))
          ...
          warning(defaultId, 'starting FieldTrip buffer on %s:%d', host, port);
  ...
  case 'brainvision_eeg'
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if append
      error(defaultId, 'appending data is not yet supported for this data format');
    end
  ...
  case 'fcdc_mysql'
    ...
    elseif isempty(hdr) && ~isempty(dat)
      ...
      error(defaultId, 'you should specify either the header or the data when writing to a MySQL database');
    end
  ...
  case 'matlab'
    ...
    if      append &&  exist(filename, 'file')
      ...
      if ~isempty(hdr) && ~isequal(hdr, prev.hdr)
        error(defaultId, 'inconsistent header');
      else
      ...
      if ~strcmp(class(buf), format{j})
        switch format{j}
          ...
          case 'uint32'
            buf = uint32(buf);
          otherwise
            error(defaultId, 'unsupported format conversion');
        end
      end
  ...
  case 'riff_wave'
    ...
    if nchans~=1
      error(defaultId, 'this format only supports single channel continuous data');
    end
  ...
  case 'plexon_nex'
    ...
    if nchans~=1
      error(defaultId, 'only supported for single-channel data');
    end
    ...
    if isfield(hdr, 'FirstTimeStamp')
      ...
      warning(defaultId, 'no timestamp information available');
  ...
  case 'neuralynx_ncs'
    ...
    elseif hdr.nChans==1
      ...
      error(defaultId, 'cannot determine channel label');
    end
