...
if isempty(inputcoord)
  error(defaultId, 'you must specify inputcoord');
end

if isempty(intersect(queryrange, [1 3 5 7 9 11]))
  error(defaultId, 'incorrect query range, should be one of [1 3 5 7 9 11]');
end
      ...
      for dj=(-(queryrange-1)/2):1:((queryrange-1)/2)
        for dk=(-(queryrange-1)/2):1:((queryrange-1)/2)
          ...
          if ijk(1)>=1 && ijk(1)<=atlas.dim(1) && ...
            ...
            warning(defaultId, 'location is outside atlas volume');
          end % k
