...
if ~nboards
  error(defaultId, 'could not determine the number of boards and channels');
end
...
elseif nargin>3 && ischar(channel)
  switch channel
    ...
    otherwise
      error(defaultId, 'unknown value in channel');
  end
elseif nargin<4
  channel = 'all';
  chanindx = 1:blocksize;
...
if needhdr
  ...
  if (beg_stx~=2048)
    error(defaultId, 'problem with STX at the begin of the file');
  end
  % check that the file is truely continuous, i.e. no gaps with junk in between
  if (end_stx~=2048)
    error(defaultId, 'problem with STX at the end of the file');
  end
...
elseif length(chanindx)>1
  ...
  if size(dat,2)<(endsample-begsample+1)
    error(defaultId, 'could not read all samples');
  end
