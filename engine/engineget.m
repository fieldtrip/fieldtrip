...
if numel(enghandle)~=1
  error(defaultId, 'FieldTrip:engine:jobNotFound', 'cannot locate the engine with this job');
end
...
if success
  ...
  if ~isempty(err)
    if StopOnError
      ...
      warning(defaultId, 'error during remote execution: %s', errmsg);
    end
  end % ~isempty(err)
  if closeline
    fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
  ...
  switch output
    ...
    case 'cell'
      ...
      varargout{2} = options;
    otherwise
      error(defaultId, 'invalid output option');
  end
  
else
  warning(defaultId, 'FieldTrip:engine:jobNotAvailable', 'the job results are not yet available');
  switch output
    case 'varargout'
