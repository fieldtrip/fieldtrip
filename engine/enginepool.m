...
switch cmd
  ...
  case 'open'
    if poolsize>1
      error(defaultId, 'you first have to close the existing pool before opening another one');
    end
  ...
  case 'block'
    ...
    if index>poolsize
      error(defaultId, 'invalid index %s, the pool only contains %d workers', index, poolsize);
    end
