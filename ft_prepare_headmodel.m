...
switch cfg.method
  ...
  case 'asa'
    if ~ft_filetype(cfg.headmodel, 'asa_vol')
      error(defaultId, 'You must supply a valid cfg.headmodel for use with ASA headmodel')
    end
  ...
  case {'bemcp' 'dipoli' 'openmeeg'}
    ...
    elseif isfield(data, 'transform') && isfield(data, 'dim')
      ...
      error(defaultId, 'Either a segmentated MRI or data with closed triangulated mesh is required as data input for the bemcp, dipoli or openmeeg method');
    end
    ...
    if strcmp(cfg.method, 'bemcp')
      ...
      if any(isnan(headmodel.mat(:)))
        ...
        warning(defaultId, 'NaN detected, trying once more with slightly different vertex positions');
        headmodel = ft_headmodel_bemcp(geometry, 'conductivity', cfg.conductivity);
      end
  ...
  case 'concentricspheres'
    ...
    elseif ~isempty(cfg.headshape) && ischar(cfg.headshape)
      ...
      error(defaultId, 'You must give a mesh, segmented MRI, sensor data type, or cfg.headshape');
    end
  ...
  case 'halfspace'
    if input_mesh || input_pos
      ...
      error(defaultId, 'a surface mesh is required as input for the halfspace method');
    end
    if isempty(cfg.point)
      error(defaultId, 'cfg.point is required for halfspace method');
    end
  ...
  case {'localspheres' 'singlesphere' 'singleshell'}
    ...
    elseif input_seg
      ...
      if ~isempty(cfg.tissue)
        ...
        if isempty(geometry)
          error(defaultId, 'please specificy cfg.tissue and pass an appropriate segmented MRI as input data')
        end
      end
    elseif input_elec
      geometry.pos = data.chanpos;
    ...
    elseif ~isempty(cfg.headmodel)
      ...
      error(defaultId, 'this requires a mesh, set of surface points or a segmented mri');
    end
    ...
    switch cfg.method
      case 'singlesphere'
        if ~isempty(cfg.headmodel)
          ...
          try
            ...
            error(defaultId, 'the volume conduction model in "%s" is invalid', cfg.headmodel);
          end
        else
      ...
      case 'localspheres'
        if ~isempty(cfg.headmodel)
          ...
          if isempty(cfg.grad)
            error(defaultId, 'for cfg.method = %s, you need to supply a cfg.grad structure', cfg.method);
          end
  ...
  case {'simbio'}
    if input_elec || isfield(data, 'pos') || input_mesh
      ...
      error(defaultId, 'You must provide a mesh with tetrahedral or hexahedral elements, where each element has a scalar or tensor conductivity');
    end
  ...
  case {'fns'}
    if input_seg
      ...
      error(defaultId, 'segmented MRI must be given as data input')
    end
    sens = ft_fetch_sens(cfg, data);
