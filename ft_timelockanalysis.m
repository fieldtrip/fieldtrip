...
if ntrial==0, error(defaultId, 'Number of trials selected in data is zero');   end
if nchan==0,  error(defaultId, 'Number of channels selected in data is zero'); end
...
switch cfg.vartrllength
  case 0
    if ~all(minperlength==maxperlength)
      error(defaultId, 'data has variable trial lengths, you specified not to accept that');
    end
  case 1
    if all(minperlength==maxperlength)
  ...
  otherwise
    error(defaultId, 'unknown value for vartrllength');
end

if strcmp(cfg.covariance, 'yes')
  if ~isfield(cfg, 'covariancewindow')
    warning(defaultId, 'the option cfg.covariancewindow is not specified, taking all time points');
  ...
  if ischar(cfg.covariancewindow)
    switch cfg.covariancewindow
      ...
      case 'all'
        cfg.covariancewindow = latency;
      case 'minperlength'
        error(defaultId, 'cfg.covariancewindow = ''minperlength'' is not supported anymore');
      case 'maxperlength'
        error(defaultId, 'cfg.covariancewindow = ''maxperlength'' is not supported anymore');
      otherwise
        error(defaultId, 'unsupported specification of cfg.covariancewindow');
    end
  end
end
  ...
  switch cfg.vartrllength
    ...
    case 1
      ...
      if strcmp(cfg.covariance,'yes') && (begsamplatency(i)>cfg.covariancewindow(1) || endsamplatency(i)<cfg.covariancewindow(2))
        usetrial = 0;
        warning(defaultId, ['trial ' num2str(i) ' not used for avg computation because it was not used for covariance computation']);
      end
    case 2
...
% compute the average
if ~any(numsamples)
  warning(defaultId, 'no samples found in the specified time window, check option for vartrllength');
end
