...
if ft_datatype(varargin{1}, 'raw+comp')
    if length(varargin)>1
        error(defaultId, 'ft_math does not support more than one input argument if the input data is of type "raw" or "comp"')
    end
end
...
for p=1:length(cfg.parameter)
  if ~issubfield(varargin{1}, cfg.parameter{p})
    error(defaultId, 'the requested parameter is not present in the data');
  end
end
...
for p = 1:length(cfg.parameter)
  ...
  if p>1 && ~strcmp(dimordtmp{1}, dimordtmp{p})
    error(defaultId, 'the dimord of multiple parameters must be the same');
  end
end
clear dimordtmp
...
for p = 1:length(cfg.parameter)
  ...
  if ~isempty(m)
    ...
    if ~ok
      error(defaultId, 'the dimensions of cfg.matrix do not allow for element-wise operations');
    end
  end
  ...
  if ~isempty(m) && ~isempty(s)
    error(defaultId, 'you can either specify a cfg.matrix or a cfg.scalar, not both');
  end
    ...
    switch cfg.operation
      ...
      case 'subtract'
        if length(varargin)>2
          error(defaultId, 'the operation "%s" requires exactly 2 input arguments', cfg.operation);
        end
