...
if numel(lags)>1 || lags~=0,
  if numel(refindx)>1, error(defaultId, 'with multiple lags, or with a lag~=0 only a single refindx is allowed'); end
...
if length(refindx)~=numel(refindx)
  ...
  error(defaultId, 'channelcmb indexing is not supported');
end
