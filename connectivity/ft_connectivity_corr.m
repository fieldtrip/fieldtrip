...
if isempty(dimord)
  error(defaultId, 'input parameters should contain a dimord');
end
...
if ~isempty(pchanindx),
  ...
  if numel(siz)==5 && siz(5)>1, error(defaultId, 'this only works for data without time'); end
...
switch str
  ...
  case '-logabs'
    c = -log(1 - abs(c).^2);
  otherwise
    error(defaultId, 'complex = ''%s'' not supported', str);
end
