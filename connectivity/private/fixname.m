...
switch version
  case 'default'
    ...
    if numel(str)>63
      str = str(1:63);
      warning(defaultId, '%s exceeds MATLAB''s maximum name length of 63 characters and has been truncated to %s', str, str(1:63));
    end
  ...
  otherwise
    error(defaultId, 'unsupported version "%s"', version);
end
