...
% or as
%   lcoh = ft_connectivity_laggedcoherence(cfg,freqout)
%
% The input data should be organised in a structure as obtained from the
% FT_FREQANALYSIS function (freqout), such that freqout contains the fields
% 'fourierspctrm' and 'time'. The timepoints must be chosen such that the desired
% cfg.lag/cfg.foi (lag in seconds) is an integer multiple of the time resolution in
% freqout.
%
% At the moment, this function must be called separately for each frequency of
% interest. To analyse multiple frequencies, we advise the use of a for-loop such as
% this:
%
%   cfg_F  = [];
%   cfg_LC = [];
...
if strcmp(cfg.autocmb,'yes')
  cfg.autocmb = true;
elseif strcmp(cfg.autocmb,'no')
  cfg.autocmb = false;
end
if ~iscell(cfg.trialsets)
  cfg.trialsets = {cfg.trialsets};
end
...
if ~isfield(freqout,'fourierspctrm')
  ft_error('this function requires frequency data with a fourierspctrm');
