...
if ~isfield(freqout,'fourierspctrm')
    error(defaultId, 'ft_connectivity_laggedcoherence requires frequency data with a fourierspctrm.');
elseif size(freqout.fourierspctrm,4)==1
    error(defaultId, 'ft_connectivity_laggedcoherence requires frequency data with a time axis')
end
...
if ~(strcmp(cfg.output,'lcoh') || strcmp(cfg.output,'csd'))
    error(defaultId, 'cfg.output must be either ''lcoh'' or ''csd''');
end
index = cellfun('isclass',cfg.trialsets,'char');
if sum(index)>0
    index2               =  cellfun(@(x) strcmp(x,'all'),cfg.trialsets(index),'UniformOutput',true);
    if any(index2==0)
        error(defaultId, 'each cell of cfg.trialsets must contain either a 1xN vector, or ''all''');
    end
...
switch cfg.timeresolved
    ...
    case 'yes'
        ...
        if cfg.nlags>1
            error(defaultId, 'when calculating timeresolved lcoh, cfg.nlags must be set to 1');
        end
