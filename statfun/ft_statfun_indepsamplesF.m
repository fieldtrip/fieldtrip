...
if isfield(cfg,'uvar') && ~isempty(cfg.uvar)
    error(defaultId, 'cfg.uvar should not exist for an independent samples statistic');
end
...
if nrepl<size(design,2)
  error(defaultId, 'Invalid specification of the independent variable in the design array.');
end
if nrepl<=ncond
    error(defaultId, 'The must be more trials/subjects than levels of the independent variable.');
end
dfnum = ncond - 1;
dfdenom = nrepl - ncond;
...
if strcmp(cfg.computecritval,'yes')
  ...
  s.dfdenom = dfdenom;
  if cfg.tail==-1
      error(defaultId, 'For an independent samples F-statistic, it does not make sense to calculate a left tail critical value.');
  end
  if cfg.tail==0
      error(defaultId, 'For an independent samples F-statistic, it does not make sense to calculate a two-sided critical value.');
  end
  if cfg.tail==1
...
if strcmp(cfg.computeprob,'yes')
  ...
  s.dfdenom = dfdenom;
  if cfg.tail==-1
      error(defaultId, 'For an independent samples F-statistic, it does not make sense to calculate a left tail p-value.');
  end
  if cfg.tail==0
      error(defaultId, 'For an independent samples F-statistic, it does not make sense to calculate a two-sided p-value.');
  end
  if cfg.tail==1
