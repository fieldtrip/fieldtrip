...
if ~ft_senstype(data, 'ctf')
  error(defaultId, 'synthetic gradients can only be computed for CTF data');
end

% check whether there are reference channels in the input data
hasref = ~isempty(ft_channelselection('MEGREF', data.label));
if ~hasref
  error(defaultId, 'synthetic gradients can only be computed when the input data contains reference channels');
end
...
if ~strcmp(current, 'none')
  ...
  try
    ...
    error(defaultId, 'unknown balancing for input data');
  end
...
if numel(selnew)==numel(labelold)
  ...
  warning(defaultId, 'channel ordering might have changed');
end
