...
if all(isspike)
  ...
  if length(unique(label))~=length(label)
    error(defaultId, 'not all channel labels are unique');
  end
  ...
  if isempty(trl);
    error(defaultId, 'could not find the trial information in the continuous data');
  end
  ...
  try
    ...
    error(defaultId, 'could not find the timestamp information in the continuous data');
  end
