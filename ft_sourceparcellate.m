...
if ~isequaln(source.pos, parcellation.pos)
  error(defaultId, 'the source positions are not consistent with the parcellation, please use FT_SOURCEINTERPOLATE');
end
...
if isempty(cfg.parcellation)
  ...
  for i=1:numel(fn)
    if isfield(parcellation, [fn{i} 'label'])
      warning(defaultId, 'using "%s" for the parcellation', fn{i});
...
if isempty(cfg.parcellation)
  error(defaultId, 'you should specify the field containing the parcellation');
end
...
if numel(fn)==0
  error(defaultId, 'there are no source parameters that can be parcellated');
end
...
for i=1:numel(fn)
    ...
    for j1=1:numel(seglabel)
      for j2=1:numel(seglabel)
        ...
        switch cfg.method
          ...
          otherwise
            error(defaultId, 'method %s not implemented for %s', cfg.method, dimord{i});
        end % switch
      end % for j2
    end % for j1
    ft_progress('close');
  ...
  elseif strncmp('pos', dimord{i}, 3)
      ...
      switch cfg.method
        ...
        case 'mean_thresholded'
          ...
          if isempty(cfg.mean.threshold)
            error(defaultId, 'when cfg.method = ''mean_thresholded'', you should specify a cfg.mean.threshold');
          end
    ...
    error(defaultId, 'unsupported dimord %s', dimord{i})
