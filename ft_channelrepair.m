...
if any(isnan(sens.chanpos(:)))
  error(defaultId, 'The channel positions contain NaNs; this prohibits correct behavior of the function. Please replace the input channel definition with one that contains valid channel positions');
end
...
if ismeg && ~any(strcmp(ft_senstype(sens), {'ctf151', 'ctf275', 'bti148', 'bti248', 'babysquid74'}))
  ...
  warning(defaultId, 'be careful when using "%s" - mixing of sensor types (e.g. magnetometers and gradiometers) can lead to wrong data. Check your neighbour-structure thoroughly', ft_senstype(sens));
end
...
if ~isempty(cfg.missingchannel) && strcmp(cfg.method, 'weighted')
  warning(defaultId, 'Reconstructing missing channels using the weighted neighbour approach is not recommended!');
end
...
elseif strcmp(cfg.method, 'spline') || strcmp(cfg.method, 'slap')
  ...
  d = mean(abs(d) / r);
  if abs(d-1) > 0.1
    warning(defaultId, 'bad spherical fit (residual: %.2f%%). The interpolation will be inaccurate.', 100*(d-1));
  elseif abs(d-1) < 0.01
  ...
  if strcmp(cfg.method, 'slap')
    warning(defaultId, '''slap'' method is not fully supported - be careful in interpreting your results');
  end
  ...
  if isempty(goodchanindcs)
    ...
    allchans = true;
    warning(defaultId, 'No good channels found - interpolating based on all channels');
  end
...
elseif strcmp(cfg.method, 'nan')
  ...
  error(defaultId, 'unknown method "%s" for interpolation', cfg.method);
end
