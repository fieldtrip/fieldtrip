...
if any(isnan(dat))
  warning(defaultId, 'the data passed to ft_plot_topo contains NaNs, these channels will be removed from the data to prevent interpolation errors, but will remain in the mask');
  flagNaN = true;
else
  flagNaN = false;
end
...
elseif ~isempty(mask)
  ...
  if ~isempty(newpoints) && (hpos == 0 || vpos == 0)
    warning(defaultId, 'Some points fall outside the outline, please consider using another layout')
...
elseif strcmp(style, 'imsat') || strcmp(style, 'imsatiso')
  ...
  if isempty(clim)
    error(defaultId, 'clim is required for style = ''imsat'' or style = ''imsatiso''')
  end
