...
% fiducials in the template mesh set.
%
%   cfg.method         = string representing the method for aligning the mesh
...
ft_preamble init
ft_preamble debug
ft_preamble loadvar    mesh_original
ft_preamble provenance mesh_original
...
if usefiducial
    ...
    %use ft_electroderealign for realign the mesh
    mesh_realigned           = ft_electroderealign(cfg);
    %use transformation obtained by ft_electroderealign
    transform                = [mesh_original.pos, ones(length(mesh_original.pos),1)]*mesh_realigned.homogeneous';
