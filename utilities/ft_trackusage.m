...
if ~isfield(ft_default, 'trackusage')
  ...
  warning(defaultId, 'enabling online tracking of FieldTrip usage, see http://www.fieldtriptoolbox.org/faq/tracking');
  if exist(fieldtripprefs, 'file')
...
if ~status
  disp(output);
  warning(defaultId, 'could not send tracker information for "%s"', event);
end
