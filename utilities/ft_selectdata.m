...
if strcmp(cfg.select, 'union') && (avgoverpos || avgoverchan || avgoverchancmb || avgoverfreq || avgovertime || avgoverrpt)
  error(defaultId, 'cfg.select ''union'' in combination with averaging across one of the dimensions is not possible');
end
    ...
    if ~isempty(selindx) && all(isnan(selindx))
      ...
      switch seldim
        ...
        case 6
          data.(datfield)(:,:,:,:,:,sel) = tmp(:,:,:,:,:,selindx(sel));
        otherwise
          error(defaultId, 'unsupported dimension (%d) for making a selection for %s', seldim, datfield);
      end
    end
...
elseif numel(selchan)>1  && any(~isfinite(selchan))
  ...
  error(defaultId, 'cannot figure out how to select channels');
end
...
if isempty(selchancmb)
  error(defaultId, 'no channel combinations were selected');
...
elseif numel(selchancmb)>1  && any(~isfinite(selchancmb))
  ...
  % this should never happen
  error(defaultId, 'cannot figure out how to select channelcombinations');
end
...
for k = 1:ndata
  ...
  if numel(unique(indx(isfinite(indx(:,k)),k)))<sum(isfinite(indx(:,k)))
    error(defaultId, 'the selection of channels across input arguments leads to double occurrences');
  end
  chanindx{k} = indx(:,k);
end
...
if ~isfield(cfg, 'channelcmb')
  ...
  switch selmode
    ...
    case 'union'
      ...
      error(defaultId, 'union of channel combination is not yet supported');
...
if ft_datatype(varargin{1}, 'spike')
  error(defaultId, 'latency selection in spike data is not supported')
end
