...
if ~isfield(source, 'inside')  || ...
   ...
   ~isfield(source, 'dim')
  error(defaultId, 'one of the required fields is missing in the source structure');
end

if ~isfield(source, 'pos') && (~isfield(source, 'xgrid') || ~isfield(source, 'ygrid') || ...
                               ~isfield(source, 'zgrid'))
  error(defaultId, 'the input data needs at least a ''pos'' field, or ''x/y/zgrid''');
end
...
if isfield(source, 'xgrid'),
  ...
  warning(defaultId, 'assuming the voxel data to be ordered as if in a regularly spaced 3D grid');
    ...
    if any(size(source.(fn{i}))==Nsparse)
      if iscell(source.(fn{i}))
        ...
        elseif all(indx==2)
          ...
          warning(defaultId, 'sparse to full conversion failed for field %s\n', fn{i});
        end
      else
        indx = find(size(source.(fn{i}))==Nsparse);
        if all(indx==1)
