function [ftver, ftpath] = ft_version(cmd)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Hidden, advanced and currently broken options:
%
% The release version of FieldTrip contains a signature file (signature.md5), that
% contains, for each file in the release, the latest revision number for that
% file, along with an MD5 hash of the file's contents. A similar signature file is
% located on the FieldTrip webserver, which is updated whenever a developer makes
% a change to the codebase.
%
% This function uses both the local signature file (dating from whenever you
% downloaded FieldTrip) and the remote signature file (reflecting the latest
% version available--this updates more frequently than the daily FTP release) to
% determine (1) the version of FieldTrip you are using, (2) whether you have made
% local changes to your FieldTrip installation (by comparing the actual MD5 hashes
% of files to the ones stored in your local signature file), (3) whether there are
% new updates available (by comparing the local and remote signature files).
%
%
...
switch cmd
  ...
  case 'full'
    ...
    if issvn
      ...
      fprintf('\nThis is FieldTrip, version %s%s (svn).\n\n', ft_version(), modFlag);
      ...
      fprintf('This is FieldTrip, version %s%s.\n', ft_version(), modFlag);
