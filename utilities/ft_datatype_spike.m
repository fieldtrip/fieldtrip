...
switch version
  case '2011'
    ...
    if isfield(spike,'origtrial') && isfield(spike,'origtime')
      ...
      warning(defaultId, 'The spike datatype format you are using is depreciated. Converting to newer spike format');
      ...
      if ~isfield(spike, 'trialtime')
        ...
        warning(defaultId, 'Reconstructing the field trialtime from spike.origtime and spike.origtrial. This is not the original representation');        
      ...
      if any(hasdat) %otherwise, ignore
        ...
        for iUnit = 1:nUnits
          ...
          if length(dim)==2 && ~isempty(spike.waveform{iUnit})
            ...
            if isempty(spikedim)
              error(defaultId, 'waveforms contains data but number of waveforms does not match number of spikes');
            end
            if spikedim==1
  ...
  otherwise
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    error(defaultId, 'unsupported version "%s" for spike datatype', version);
end
