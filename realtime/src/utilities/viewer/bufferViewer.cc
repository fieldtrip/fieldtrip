...
void idleCall(void *dummy) {
   ...
   // wait for new samples, don't care about events, up to 40ms
	...
	if (ddef.data_type == DATATYPE_FLOAT32) {
		if (useHighpass) {
			hpFilter->process(ddef.nsamples, (float *) floatStore.data(), (float *) rawStore.data());
			oddis->addSamples(ddef.nsamples, (float *) floatStore.data());			
		} else {
			oddis->addSamples(ddef.nsamples, (float *) rawStore.data());
		}
	} else {
...
int main(int argc, char **argv) {
	...
	addrField = new Fl_Input(160,10,210,20,"Buffer address");
	addrField->textsize(12);
	addrField->textfont(FL_COURIER);
	addrField->labelsize(12);
	
	conButton = new Fl_Button(380,10,100,20,"Connect");
	conButton->labelsize(12);
	conButton->callback(connectCallback);
	
	hpButton = new Fl_Check_Button(500,10,100,20,"Highpass filter");
