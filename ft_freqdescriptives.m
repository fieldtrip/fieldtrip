...
if sum([varflg keepflg]>1),               error(defaultId, 'you should specify only one of cfg.keeptrials or cfg.variance');                                             end
if ~hasrpt && (varflg || keepflg),        error(defaultId, 'a variance-estimate or a single trial estimate without repeated observations in the input is not possible'); end
if ~hasrpt && ~strcmp(cfg.trials, 'all'), error(defaultId, 'trial selection requires input data with repeated observations');                                            end
