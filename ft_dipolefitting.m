...
if ~isempty(cfg.symmetry)
  if cfg.numdipoles~=2
    error(defaultId, 'symmetry constraints are only supported for two-dipole models');
...
if ft_getopt(cfg.dipfit.constr, 'sequential', false) && strcmp(cfg.model, 'moving')
  error(defaultId, 'the moving dipole model does not combine with the sequential constraint')
...
if nchans<cfg.numdipoles*3
  warning(defaultId, 'not enough channels to perform a dipole fit');
end

if ntime<1
  error(defaultId, 'no spatial topography selected');
end

% check whether EEG is average referenced
if ft_senstype(sens, 'eeg')
  if any(rv(Vdata, avgref(Vdata))>0.001)
    warning(defaultId, 'the EEG data is not average referenced, correcting this');
  end
  Vdata = avgref(Vdata);
end
...
if numel(cfg.dip.pos)~=cfg.numdipoles*3 || numel(cfg.dip.mom)~=cfg.numdipoles*3
  error(defaultId, 'inconsistent number of dipoles in configuration')
end
...
if strcmp(cfg.gridsearch, 'yes')
  ...
  elseif isfield(cfg.grid, 'pos') && size(cfg.grid.pos,2)==cfg.numdipoles*3
    ...
    error(defaultId, 'dipole scanning is only possible for a single dipole or a symmetric dipole pair');
  end
  ...
  switch cfg.model
    ...
    otherwise
      error(defaultId, 'unsupported cfg.model');
  end
...
switch cfg.model
  ...
  case 'moving'
    if isfreq
      ...
      warning(defaultId, 'a moving dipole model in the frequency domain is not supported');
    end
  otherwise
    error(defaultId, 'unsupported cfg.model');
end % switch model
