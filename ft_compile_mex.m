...
% Note that this function does not put the MEX files in the correct location,
% this is managed by SVN autosync.
%
% Please note that this script does NOT set up your MEX environment for you, so
% in case you haven't selected the C compiler on Windows yet, you need to type
% 'mex -setup' first to choose either the Borland or Microsoft compiler. If you
% want to use MinGW, you also need to install Gnumex
% (http://gnumex.sourceforget.net), which comes with its own procedure for
% setting up the MEX environment.
%
% The logic in this script is to first build a list of files that actually need
% compilation for the particular platform that Matlab is running on, and then to
% go through that list. Functions are added to the list by giving their
% destination directory and (relative to that) the name of the source file
% (without the .c). Optionally, you can specify a list of platform this file
% needs to be compiled on only, and a list of platforms where you don't compile
% it on. Finally, you can give extra arguments to the MEX command, e.g., for
% including other c-sources or giving compiler flags.
%
% See also MEX
...
% ADD_MEX_SOURCE is a helper function for compiling mex files
%
% Use as
