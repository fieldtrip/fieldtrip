...
% cfg.order
% cfg.vol, documented
% cfg.online , it should be 1 only if you want to run an online experiment

% cfg.onlineprocess, keeps the transformation martix to use for calling
% MEGPLANAR again in the online paradigm
% In the online paradigm, each time that you want to call MEGPLANAR, you need to copy the previous
% onlineprocess and call MEGPLANAR after that. If you call megplanar like:
% cfgpl.online=1;
% plan = megplanar(cfgpl,data); ,,the copy is like:
% cfgpl.onlineprocess=plan.cfg.onlineprocess;

% Copyright (C) 2004, Robert Oostenveld
% Subversion does not use the Log keyword, use 'svn log <filename>' or 'svn -v log | less' to get detailled information

% 24 November, alibah added: cfg.online, cfg.onlineprocess

%to avoid repeating steps for online paradigm
if ~isfield(cfg, 'onlineprocess') || ~(isfield(cfg.onlineprocess,'transform'))|| ~isfield(cfg.onlineprocess, 'interp')
    ...
    if cfg.online
        cfg.onlineprocess=[];
        cfg.onlineprocess.isfreq=isfreq;
        cfg.onlineprocess.israw=israw;
        cfg.onlineprocess.istlck=istlck;
    end
    ...
    if israw || istlck,
        ...
        if cfg.online
            cfg.onlineprocess.Ntrials=Ntrials;
        end
    elseif isfreq,
    ...
    elseif strcmp(cfg.planarmethod, 'sincos')
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % This attempts to re-implements Ole's method, except that the definition of the
    ...
    isfreq=cfg.onlineprocess.isfreq;
    israw=cfg.onlineprocess.israw;
    istlck=cfg.onlineprocess.istlck;
    Ntrials=cfg.onlineprocess.Ntrials;
    if isfield(cfg.onlineprocess,'Nfreq')
        Nfreq=cfg.Nfreq;
    end
end
...
if ~strcmp(cfg.planarmethod, 'sourceproject')
    if ~isfield(cfg.onlineprocess, 'transform') || ~isfield(cfg.onlineprocess, 'interp')
        ...
        interp.grad = planar.grad;
        if cfg.online
            cfg.onlineprocess.transform=transform;
            cfg.onlineprocess.interp=interp;
        end
    else
        interp=cfg.onlineprocess.interp;
        transform=cfg.onlineprocess.transform;
    end
...
if ~cfg.online
    ...
    %%cfg = checkconfig(cfg, 'trackconfig', 'off'); %%checksize will remove gradH and gradV
end
