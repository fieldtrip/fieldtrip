...
if ~isempty(cfg.coordsys) && isempty(cfg.target)
  ...
  switch lower(cfg.coordsys)
    ...
    otherwise
      error(defaultId, 'the %s coordinate system is not automatically supported, please specify fiducial details in cfg.target')
  end
end
...
if strcmp(cfg.method, 'fiducial') && isfield(cfg, 'warp') && ~isequal(cfg.warp, 'rigidbody')
  warning(defaultId, 'The method ''fiducial'' implies a rigid body tramsformation. See also http://bugzilla.fieldtriptoolbox.org/show_bug.cgi?id=1722');
  cfg.warp = 'rigidbody';
end
if strcmp(cfg.method, 'fiducial') && isfield(cfg, 'warp') && ~isequal(cfg.warp, 'rigidbody')
  warning(defaultId, 'The method ''interactive'' implies a rigid body transformation. See also http://bugzilla.fieldtriptoolbox.org/show_bug.cgi?id=1722');
  cfg.warp = 'rigidbody';
end
...
elseif strcmp(cfg.method, 'headshape')
  ...
  elseif strcmp(cfg.warp, 'fsaverage')
    ...
    if ~isdir([cfg.fshome filesep 'subjects' filesep 'fsaverage' filesep 'surf'])
      error(defaultId, ['freesurfer dir ' cfg.fshome filesep 'subjects' filesep 'fsaverage' filesep 'surf cannot be found'])
    end
...
elseif strcmp(cfg.method, 'fiducial')
  ...
  if ~isfield(cfg, 'fiducial') || isempty(cfg.fiducial)
    ...
    elseif length(match_str(label, option6))==3
      ...
      error(defaultId, 'could not determine consistent fiducials in the input and the target, please specify cfg.fiducial or cfg.coordsys')
    end
  end
  ...
  if length(cfg.fiducial)~=3
    error(defaultId, 'you must specify exactly three fiducials');
  end
  ...
  if length(nas_indx)~=1 || length(lpa_indx)~=1 || length(rpa_indx)~=1
    error(defaultId, 'not all fiducials were found in the electrode set');
  end
  ...
  for i=1:Ntemplate
    ...
    if length(nas_indx)~=1 || length(lpa_indx)~=1 || length(rpa_indx)~=1
      error(defaultId, 'not all fiducials were found in template %d', i);
    end
