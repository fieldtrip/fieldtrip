...
if ~isfield(mri, 'anatomy')
  error(defaultId, 'no anatomical information available, this is required for normalisation');
end
...
if isempty(cfg.template)
  error(defaultId, 'you must specify a template anatomical MRI');
end
...
if strcmp(template_ftype, 'analyze_hdr') || strcmp(template_ftype, 'analyze_img') || strcmp(template_ftype, 'minc') || strcmp(template_ftype, 'nifti')
  ...
  error(defaultId, 'the head coordinate system of the template does not seem to be correspond with the mni/spm convention');
end
...
switch template_ftype
  ...
  otherwise
    error(defaultId, 'Unknown template');
end
