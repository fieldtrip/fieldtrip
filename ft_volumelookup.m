...
elseif isfield(cfg, 'maskparameter')
  ...
  error(defaultId, 'you should either specify cfg.roi, or cfg.maskparameter')
end
...
if roi2mask
  ...
  if isatlas+ispoi ~= 1
    error(defaultId, 'do not understand cfg.roi')
  end
  ...
  elseif ispoi
    ...
    if isempty(ft_getopt(cfg, 'sphere')) && isempty(ft_getopt(cfg, 'box'))
      error(defaultId, 'you should either specify cfg.sphere or cfg.box')
    end
  end
...
elseif mask2label || roi2label
  ...
  if isempty(intersect(cfg.maxqueryrange, [1 3 5]))
    error(defaultId, 'incorrect query range, should be one of [1 3 5]');
  end
  ...
  if isatlas
    ...
    if isindexed
      ...
      elseif ~strcmp(cfg.inputcoord, atlas.coordsys)
        error(defaultId, 'there is a mismatch between the coordinate system in the atlas and the coordinate system in the data, which cannot be resolved');
      end
      ...
      error(defaultId, 'support for atlases that have a probabilistic segmentationstyle is not supported yet');
