function rhs = sb_rhs_venant(pos,dir,node,elem);
%find node closest to source position
next_nd = sb_get_next_nd(pos,node);
%find nodes neighbouring closest node
ven_nd = sb_get_ven_nd(next_nd,elem);
%calculate rhs matrix
loads = sb_calc_ven_loads(pos,dir,ven_nd,node);
...
loads = loads(i~=0);
i = i(i~=0);
rhs = sparse(i,j,loads,size(node,1),size(pos,1));
end
