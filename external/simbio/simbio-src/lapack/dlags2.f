...
*          On entry, A1, A2 and A3 are entries of the input 2-by-2
*          upper (lower) triangular matrix A.
*
*  B1      (input) DOUBLE PRECISION
*  B2      (input) DOUBLE PRECISION
*  B3      (input) DOUBLE PRECISION
*          On entry, B1, B2 and B3 are entries of the input 2-by-2
...
*     ..
*     .. Local Scalars ..
      DOUBLE PRECISION   A, AUA11, AUA12, AUA21, AUA22, AVB12, AVB21,
     $                   AVB22, B, C, CSL, CSR, D, R, S1, S2, SNL, SNR,
     $                   UA11, UA11R, UA12, UA21, UA22, UA22R, VB11,
     $                   VB11R, VB12, VB21, VB22, VB22R
*     ..
...
*
     ...
     $        THEN
*
*           Compute the (1,1) and (1,2) entries of U'*A and V'*B,
*           and (1,2) entry of |U|'*|A| and |V|'*|B|.
*
            UA11R = CSL*A1
            UA12 = CSL*A2 + SNL*A3
*
...
*
*           zero (1,2) entries of U'*A and V'*B
*
...
*
         ELSE
*
*           Compute the (2,1) and (2,2) entries of U'*A and V'*B,
*           and (2,2) entry of |U|'*|A| and |V|'*|B|.
*
            UA21 = -SNL*A1
            UA22 = -SNL*A2 + CSL*A3
*
...
*
*           zero (2,2) entries of U'*A and V'*B, and then swap.
*
...
*
            ...
            AVB21 = ABS( SNL )*ABS( B1 ) + ABS( CSR )*ABS( B2 )
*
*           zero (2,2) entries of U'*A and V'*B.
*
...
*
*           zero (1,2) entries of U'*A and V'*B, and then swap.
*
            IF( ( ABS( UA11 )+ABS( UA12 ) ).NE.ZERO ) THEN
               IF( AUA11 / ( ABS( UA11 )+ABS( UA12 ) ).LE.AVB12 /
