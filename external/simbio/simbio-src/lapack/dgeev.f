...
*
*  The left eigenvectors of A are the same as the right eigenvectors of
*  A**T.  If u(j) and v(j) are the left and right eigenvectors,
*  respectively, corresponding to the eigenvalue lambda(j), then
*  (u(j)**T)*A = lambda(j)*(u(j)**T) and A*v(j) = lambda(j) * v(j).
*
...
*        (Workspace: need 4*N)
*
         CALL DTREVC( SIDE, 'O', SELECT, N, A, LDA, VL, LDVL, VR, LDVR,
