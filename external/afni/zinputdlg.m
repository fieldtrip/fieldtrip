function Answer=zinputdlg(Prompt, Title, NumLines, DefAns, Resize)
...
%
%  ZINPUTDLG uses UIWAIT to suspend execution until the user responds.
%
%  ANSWER = ZINPUTDLG(PROMPT,NAME) specifies the title for the dialog.
%
%  ANSWER = ZINPUTDLG(PROMPT,NAME,NUMLINES) specifies the number of lines for
...
%
%  ANSWER = ZINPUTDLG(PROMPT,NAME,NUMLINES,DEFAULTANSWER) specifies the
...
%  strings.
%
%  ANSWER = ZINPUTDLG(PROMPT,NAME,NUMLINES,DEFAULTANSWER,OPTIONS) specifies
...
%
%  answer=zinputdlg(prompt,name,numlines,defaultanswer);
%
%  options.Resize='on';
%  options.WindowStyle='normal';
%  options.Interpreter='tex';
%
%  answer=zinputdlg(prompt,name,numlines,defaultanswer,options);
%
...
TxtHeight=QuestHeight(1)/size(WrapQuest{1,1},1)+10;
...
for lp=1:NumQuest,
  ...
  EditHandle(lp)=uicontrol(InputFig    , ...
   ...
   %force the damned thing to respond on 'enter' too
   if (NumLines(lp,1) == 1),
      %set (EditHandle(lp), 'KeyPressFcn',@doEditKeyPress);
      set (EditHandle(lp), 'Callback', @doCallback);
   end
...
set(InputFig,'Position',zgetnicedialoglocation(FigPos,get(InputFig,'Units')));
...
zsetdefaultbutton(InputFig, OKHandle);
...
function doEditKeyPress(obj, evd) %#ok
switch(evd.Key)
  case {'return'}
      set(gcbf,'UserData','OK');
      uiresume(gcbf);
...
% A local version of matlab's getnicedialoglocation
function figure_size = zgetnicedialoglocation(figure_size, figure_units)
...
% A local version of matlab's setdefaultbutton
function zsetdefaultbutton(figHandle, btnHandle)
