...
$Id: mxInfo.c 2782 2011-02-03 11:00:18Z roboos $
...
void printMxInfoSummary(FILE* fd, const MxInfo info){
  ...
  fprintf(fd,"]");
  if ( info.dtype==SINGLE_DTYPE ) fprintf(fd," (single"); 
  else if ( info.dtype==DOUBLE_DTYPE ) fprintf(fd," (double");
  else if ( info.dtype==LOGICAL_DTYPE ) fprintf(fd," (logical");
  if ( info.ip!=0 ) fprintf(fd," complex)"); else fprintf(fd,")");
}
...
int sizeof_dtype(MxInfoDTypes dtype){
  int dsz=0;
  switch( dtype ){
...
int dsz_bytes(const MxInfo info){ return sizeof_dtype(info.dtype); }
