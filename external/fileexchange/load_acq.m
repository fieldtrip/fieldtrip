...
function data = read_acq(fid, hdr, chan_by_chan)
  ...
  if any([hdr.per_chan_data.var_sample_divider] < 0) % Remove negative offset (not sure why that is the case???)
      hdr.per_chan_data = hdr.per_chan_data(~([hdr.per_chan_data.var_sample_divider] < 0))
      hdr.per_chan_type = hdr.per_chan_type(~([hdr.per_chan_data.var_sample_divider] < 0))
      hdr.graph.num_channels = length(~([hdr.per_chan_data.var_sample_divider] < 0));
   end
