function test_ft_freqanalysis2(datainfo, writeflag, version, breakonerror)
...
for k = 1:numel(datainfo)
  ...
  [ok,msg] = isalmostequal(freq, datanew, 'reltol', 1e-3);
  if ~ok && breakonerror
    error('stored and computed data not identical: %s\n', msg{:});
  elseif ~ok
    warning('stored and computed data not identical: %s\n', msg{:});
  end
...
if contains(output, 'csd')
  load(cfg.inputfile);
  cfg.channelcmb = [data.label repmat(data.label(3), [length(data.label), 1])];
end
