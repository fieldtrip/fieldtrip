function tests = test_ft_preproc_smooth

% MEM 1gb
% WALLTIME 00:10:00
% DEPENDENCY ft_preproc_smooth
...
result = [];
result{end+1} = ft_preproc_smooth(dat, 1);
result{end+1} = ft_preproc_smooth(dat, 2);
result{end+1} = ft_preproc_smooth(dat, 4);
result{end+1} = ft_preproc_smooth(dat, 8);
