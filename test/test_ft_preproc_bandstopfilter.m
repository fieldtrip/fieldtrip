...
for i1 = 1:numel(filttypes)
  if isequal(filttypes{i1}, 'brickwall')
    ...
    result{end+1} = ft_preproc_bandstopfilter(dat, Fs, Fbs, [], filttypes{i1}, [], instabilityfix, df, wintype, dev, plotfiltresp, usefftfilt);
    continue;
  ...
  for i2 = sel2
    for i3 = sel3
      ...
      result{end+1} = ft_preproc_bandstopfilter(dat, Fs, Fbs, filtorders{i3}, filttypes{i1}, filtdirs{i2}, instabilityfix, df, wintype, dev, plotfiltresp, usefftfilt);
    end
  end
end
