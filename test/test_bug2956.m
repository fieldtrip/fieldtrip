...
len_sec = 50;  % length of signal in seconds
Fs      = 1000; % SR
Tm      = 1/Fs;  
Ln      = round(Fs.*len_sec);
...
cfg.dftreplace   = 'neighbour_fft'; % implicates spectrum interpolation
...
figure; plot(timepts, data.trial{1},'k');
hold; plot(timepts,data_intpl.trial{1}, 'r');
...
figure;  plot(timepts, data.trial{1},'k');
hold; plot(timepts,data_dftfilt.trial{1}, 'r');
...
%% now try spectral interpolation option, with dft filtering

cfg            = [];
cfg.dftfilter  = 'yes';
cfg.dftfreq    = [50 100 150]; %
cfg.dftreplace = 'neighbour'; % implies dft filter (formerly the only option)
cfg.dftbandwidth = [1 1 1]; 
cfg.dftneighbourwidth = [1 1 1]; 
data_dftfilt   = ft_preprocessing(cfg, data);
