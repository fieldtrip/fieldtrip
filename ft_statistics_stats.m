...
case {'ttest', 'ttest_samples_vs_const'}
  ...
  if ~any(size(design)==1)
    error(defaultId, 'design matrix should only contain one factor (i.e. one row)');
  end
  Ncond = length(unique(design));
  if Ncond>1
    error(defaultId, 'method "%s" is only supported for one condition', cfg.statistic);
  end
  Nobs  = size(dat, 1);
...
case {'ttest2', 'ttest_2samples_by_timepoint'}
  ...
  Ncond = length(unique(design));
  if Ncond~=2
    error(defaultId, '%s method is only supported for two conditions', cfg.statistic);
  end
  Nobs  = size(dat, 1);
...
case {'paired-ttest'}
  ...
  Ncond = length(unique(design));
  if Ncond~=2
    error(defaultId, 'method "%s" is only supported for two conditions', cfg.statistic);
  end
  Nobs  = size(dat, 1);
  selA = find(design==design(1));
  selB = find(design~=design(1));
  Nrepl = [length(selA), length(selB)];
  if Nrepl(1)~=Nrepl(2)
    error(defaultId, 'number of replications per condition should be the same');
  end
...
case 'ttest_window_avg_vs_const'
  ...
  error(defaultId, '%s is not supported any more, use cfg.avgovertime=''yes'' instead', cfg.statistic);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
otherwise
  error(defaultId, 'method ''%s'' is not implemented', cfg.statistic);
end
