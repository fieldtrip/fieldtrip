...
switch type
  ...
  case 'firws'
    ...
    % Reporting
    ft_info(sprintf('Highpass filtering data: %s, order %d, %s-windowed sinc FIR\n', dir, order, wintype));
    if ~isTwopass && ~isOrderLow % Do not report shifted cutoffs
      ft_info(sprintf('  cutoff (-6 dB) %g Hz\n', Fhp));
      tb = [max([Fhp - df / 2 0]), min([Fhp + df / 2 Fn])]; % Transition band edges
      ft_info(sprintf('  transition width %.1f Hz, stopband 0-%.1f Hz, passband %.1f-%.0f Hz\n', df, tb, Fn));
    end
    if ~isOrderLow
