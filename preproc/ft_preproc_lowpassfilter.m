...
switch type
    ...
    case 'brickwall'
        ax = linspace(0function [filt] = ft_preproc_lowpassfilter(dat,Fs,Flp,N,type,dir,instabilityfix)
...
%                'fir' FIR filter using Matlab fir1 function
%                'firls' FIR filter using Matlab firls function (requires Matlab Signal Processing Toolbox)
%                'brickwall' Brickwall filter using Matlab FFT and iFFT function
%   dir        optional filter direction, can be
%                'onepass'         forward filter only
%                'onepass-reverse' reverse filter only, i.e. backward in time
%                'twopass'         zero-phase forward and reverse filter (default)
...
try
    ...
    switch instabilityfix
        ...
        case 'reduce'
            ...
            filt = ft_pfunction [filt] = ft_preproc_lowpassfilter(dat,Fs,Flp,N,type,dir,instabilityfix)
...
filt = bsxfun(@plus, filt, meandat);reproc_lowpassfilter(dat,Fs,Flp,N-1,type,dir,instabilityfix);
        case 'split'
            N1 = ceil(N/2);
            N2 = floor(N/2);
            warning('backtrace', 'off')
...
% add the mean back to the filtered data
filt = bsxfun(@plus, filt, meandat);, Fs, size(dat,2)); % frequency coefficients
        fh = nearest(ax, Flp)+1; % high cut-off frequency
        a  = 0; % suppresion rate of frequencies-not-of-interest
        f           = fft(dat,[],2); % FFT
        f(:,fh:end) = a.*f(:,fh:end); % perform high cut-off
