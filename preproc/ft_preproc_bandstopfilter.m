...
switch type
  ...
  case 'firws'
    ...
    % Reporting
    ft_info(sprintf('Bandstop filtering data: %s, order %d, %s-windowed sinc FIR\n', dir, order, wintype));
    if ~isTwopass && ~isOrderLow % Do not report shifted cutoffs
      ...
      ft_info(sprintf('  transition width %.1f Hz, passband 0-%.1f Hz, stopband %.1f-%.1f Hz, passband %.1f-%.0f Hz\n', df, tb, Fn));
    end
    if ~isOrderLow
