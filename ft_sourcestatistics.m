...
if false % disabled
  ...
  elseif strcmp(cfg.implementation, 'new')
    ...
    if any(strcmp(cfg.method, {'parametric' 'randomization' 'randcluster'}))
      ...
      if ~isempty(cfg.roi)
        ...
        if strcmp(cfg.avgoverroi, 'no')
          ...
          varargin{1}.inside  = varargin{1}.inside(:) & roimask(:);
    ...
    if isempty(cfg.roi) || strcmp(cfg.avgoverroi, 'no')
      ...
      stat = copyfields(varargin{1}, stat, {'dim', 'transform', 'pos', 'inside'});
    else
      stat.inside  = true(size(roilabel));
      stat.label   = roilabel(:);
    end
