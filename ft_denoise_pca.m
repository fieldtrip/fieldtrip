...
if strcmp(cfg.pertrial, 'yes')
  ...
  elseif ischar(cfg.truncate) || (cfg.truncate>1 && cfg.truncate/round(cfg.truncate)~=1) || cfg.truncate>length(refindx)
    error(defaultId, 'cfg.truncate should be either ''no'', an integer number <= the number of references, or a number between 0 and 1');
  ...
  if computeweights
    ...
    if length(i2)~=length(cfg.pca.reflabel)
      error(defaultId, 'you specified fewer references to use as there are in the precomputed weight table');
    end
...
if ~iscell(x) || length(nx)>2 || all(nx>1)
  error(defaultId, 'incorrect input for cellmean');
end
...
if nargin==1
  ...
  else   error(defaultId, 'no dimension to compute covariance for');
  end
end
...
if nargin==1
  ...
  if     all(scx2==scx2(1)), dim = 2; %let second dimension prevail
  elseif all(scx1==scx1(1)), dim = 1;
  else   error(defaultId, 'no dimension to compute mean for');
  end
end
...
nx = size(x);
if ~iscell(x) || length(nx)>2 || all(nx>1)
  error(defaultId, 'incorrect input for cellstd');
end
...
if isempty(bsxfun_exists)
  ...
  if ~bsxfun_exists
    error(defaultId, 'bsxfun not found.');
  end
end
...
else   error(defaultId, 'inconsistent input');
end
