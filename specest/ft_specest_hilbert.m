...
filttype  = ft_getopt(varargin, 'filttype');    if isempty(filttype),  error(defaultId, 'you need to specify filter type'),         end
filtorder = ft_getopt(varargin, 'filtorder');   if isempty(filtorder), error(defaultId, 'you need to specify filter order'),        end
filtdir   = ft_getopt(varargin, 'filtdir');     if isempty(filtdir),   error(defaultId, 'you need to specify filter direction'),    end
...
for ifreqoi = 1:nfreqoi
  ...
  if all((sign(tmpfreq) == 1))
    filtfreq(end+1,:) = tmpfreq;
      ...
      warning(defaultId, sprintf('frequency %.2f Hz cannot be estimated with resolution %.2f Hz', freqoi(ifreqoi), width(ifreqoi)));
  end
end
