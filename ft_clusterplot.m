...
if ~iscell(cfg.highlightseries)
  error(defaultId, 'cfg.highlightseries should be a cell-array of strings')
end
...
switch dimord
  ...
  case 'chan_freq_time'
    ...
    elseif dimsiz(3)==1
      ...
      error(defaultId, 'this only works if either frequency or time is a singleton dimension');
    end

  otherwise
    error(defaultId, 'unsupported dimord %s', dimord);
end % switch dimord
...
if issubfield(stat, 'cfg.correcttail') && ((strcmp(stat.cfg.correcttail,'alpha') || strcmp(stat.cfg.correcttail,'prob')) && (stat.cfg.tail == 0));
  if ~(cfg.alpha >= stat.cfg.alpha);
    warning(defaultId, ['the pvalue you plot: cfg.alpha = ' num2str(cfg.alpha) ' is higher than the correcttail option you tested: stat.cfg.alpha = ' num2str(stat.cfg.alpha)]);
  end
end
...
if haspos == 0 && hasneg == 0
  ...
  if Nsigall == 0
    error(defaultId, 'no clusters present with a p-value lower than the specified alpha, nothing to plot')
  end
