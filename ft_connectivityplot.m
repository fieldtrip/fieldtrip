...
for k = 1:Ndata
  ...
  switch varargin{k}.dimord
    ...
    otherwise
      error(defaultId, 'the data should have a dimord of %s or %s', 'chan_chan_freq', 'chancmb_freq');
  end
...
if hasfreq && hastime
  if Ndata>1
    error(defaultId, 'when the input data contains time-frequency representations, only a single data argument is allowed');
  end
  xparam = 'time';
  yparam = 'freq';
elseif hasfreq
...
if ~isfield(data, cfg.parameter{1})
  error(defaultId, 'the data does not contain the requested parameter %s', cfg.parameter{1});
end
...
for k = 1:nchan
  for m = 1:nchan
    if k~=m
      ...
      elseif hastime
        error(defaultId, 'plotting data with only a time axis is not supported yet');
      end
      if k==1
