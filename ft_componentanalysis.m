...
if isfield(cfg, 'topo') && isfield(cfg, 'topolabel')
  warning(defaultId, ['Specifying cfg.topo (= mixing matrix) to determine component '...
...
if isfield(cfg, 'unmixing') && isfield(cfg, 'topolabel')
  ...
  if length(toposel)~=length(cfg.topolabel)
    error(defaultId, 'not all channels that are required for the unmixing are present in the data');
  end
...
switch cfg.method
  ...
  case 'bsscca'
    ...
    if strcmp(cfg.cellmode, 'no')
      error(defaultId, 'cfg.mehod = ''bsscca'' requires cfg.cellmode = ''yes''');
    end
  otherwise
    % do nothing
end
...
elseif strcmp(cfg.method, 'csp')
  ...
  if min(length(sel1), length(sel2)) == 0
    error(defaultId, 'CSP requires class labels!');
  end
  if length(sel1)+length(sel2)~=length(cfg.csp.classlabels)
    warning(defaultId, 'not all trials belong to class 1 or 2');
  end
...
switch cfg.method
  ...
  case 'icasso'
    ...
    elseif strcmp(cfg.icasso.method, 'dss')
      ...
      error(defaultId, 'only ''fastica'' or ''dss'' is supported as method for icasso');
    end
  ...
  case 'fastica'
    ...
    elseif ~defaultNumCompsUsed &&...
      ...
      error(defaultId, 'you can specify either cfg.fastica.numOfIC or cfg.numcomponent (they will have the same effect), but not both');
    end
  ...
  case 'runica'
    ...
    elseif ~defaultNumCompsUsed &&...
      ...
      % unsure which one to use
      error(defaultId, 'you can specify either cfg.runica.pca or cfg.numcomponent (they will have the same effect), but not both');
    end
  ...
  case 'binica'
    ...
    elseif ~defaultNumCompsUsed &&...
      ...
      % unsure which one to use
      error(defaultId, 'you can specify either cfg.binica.pca or cfg.numcomponent (they will have the same effect), but not both');
    end
  ...
  case 'sobi'
    ...
    elseif isfield(cfg.sobi, 'n_sources')
      ...
      error(defaultId, 'unknown options for SOBI component analysis');
    end
  ...
  case 'predetermined unmixing matrix'
    ...
    if length(cfg.topolabel)<length(chansel)
      error(defaultId, 'cfg.topolabels do not uniquely correspond to data.label, please check')
    end
  ...
  case 'parafac'
    error(defaultId, 'parafac is not supported anymore in ft_componentanalysis');
    
  otherwise
    error(defaultId, 'unknown method for component analysis');
end % switch method
...
elseif isempty(mixing) && isempty(unmixing)
  ...
  error(defaultId, 'the component unmixing failed');
end
