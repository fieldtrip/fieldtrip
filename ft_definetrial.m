...
% and optionally
%   cfg.representation = [], 'numeric' or 'table', determines how the trl is returned (default = [], this returns the output of the trialfun as is)
...
% FT_TRIALFUN_GENERAL, which is specified as the default. It supports the
% following options
...
%
% If you want to read all data from a continuous file in a single or in multiple
% segments, FT_TRIALFUN_GENERAL understands the following options
...
%
% If you specify cfg.trialdef.eventtype='show', then FT_TRIALFUN_GENERAL will display
% a list with the events in your data on screen.
%
% If you specify cfg.trialdef.eventtype='gui', then FT_TRIALFUN_GENERAL will display
% a graphical dialog that allows you to specify the events.
%
% In general, a call to FT_DEFINETRIAL results in the trial definition "trl" being
% added to the output configuration structure. The trials are defined according to
% the triggers, trials or other events in the data, or from a user-specified MATLAB
% function (subsequently referred to as the trial function) which returns "trl". The
% user can specify the name of his/her custom trial function that is tailored to the
% experimental paradigm, or use the default trial function FT_TRIALFUN_GENERAL.
%
...
%
% The trial definition "trl" can contain additional columns besides the required
% three that represent start, end and offset. These additional columns can be used by
% a custom trialfun to provide numeric information about each trial such as trigger
% codes, response latencies, trial type and response correctness. The additional
% columns of the "trl" matrix will be represented in the "trialinfo" field after
% FT_PREPROCESSING.
%
% If FT_TRIALFUN_GENERAL has been used to generate the "trl" matrix or table, the
% function may return a fourth column in trl, that refers to the event-code for the
% corresponding trial. Whether or not this column is returned depends on the
% acquisition system. In general, this fourth column is generated by default if the
% event codes are represented numerically, or as a string starting with 'S' or 'R'
% (for BrainVision data).
%
% If you need to define the segments of interest on the basis of a conditional
% sequence of events (e.g. stimulus trigger followed by a correct response), you
% should supply your own trial function and specify that as cfg.trialfun. See below
% for pointers to some examples.
%
% The cfg.trialfun option is a string containing the name of a function that you
% wrote yourself and that FT_DEFINETRIAL will call. The function takes the cfg
% structure as input and should return a NxM matrix (with M equal to or larger than
% 3) in the same format as "trl" as the output. You can add extra custom fields to
% the configuration structure to pass as arguments to your own trialfun. Furthermore,
% inside the trialfun you can use the FT_READ_EVENT function to get the event
% information from your data file.
%
% See also FT_PREPROCESSING, FT_READ_HEADER, FT_READ_EVENT, FT_TRIALFUN_GENERAL,
% FT_TRIALFUN_BIDS, FT_TRIALFUN_EXAMPLE1, FT_TRIALFUN_EXAMPLE2
...
if size(trl,1)<1
  if (issubfield(cfg, 'trialdef.eventtype') && isequal(cfg.trialdef.eventtype, 'show')) || (issubfield(cfg, 'trialdef.eventvalue') && isequal(cfg.trialdef.eventvalue, 'show')) || (issubfield(cfg, 'trialdef.eventtype') && isequal(cfg.trialdef.eventtype, '?')) || (issubfield(cfg, 'trialdef.eventvalue') && isequal(cfg.trialdef.eventvalue, '?'))
