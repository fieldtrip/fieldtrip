...
% trial relative to the start of the raw data, the second column contains the
% sample-indices of the end of each trial, and the third column contains the
% offset of the trigger with respect to the trial. An offset of 0 means that
% the first sample of the trial corresponds to the trigger. A positive offset
% indicates that the first sample is later than the trigger, and a negative
% offset indicates that the trial begins before the trigger.
%
% The trial definition "trl" can contain additional columns besides the
% required three that represent start, end and offset. These additional
...
%
% If FT_TRIALFUN_GENERAL has been used to generate the "trl" matrix or table,
% the function may return a fourth column in trl, that refers to the event-code
% for the corresponding trial. Whether or not this column is returned depends
% on the acquisition system. In general, this fourth column is generated by
% default if the event codes are represented numerically, or as a string
% starting with 'S' or 'R' (for BrainVision data).
%
