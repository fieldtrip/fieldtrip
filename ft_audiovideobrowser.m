...
elseif hasdata
  ...
  error(defaultId, 'the data header is not available')
end
...
elseif hasdata && isfield(data, 'sampleinfo')
  ...
  error(defaultId, 'the EEG/MEG data segments should be specified');
end
...
while (true)
  ...
  if istrue(cfg.interactive)
    ...
    switch response
      case 'n'
        if trllop==numtrl
          warning(defaultId, 'already at the last trial');
        else
          trllop = trllop+1;
        end
      case 'p'
        if trllop==1
          warning(defaultId, 'already at first trial');
        else
          trllop = trllop-1;
        end
      case 'q'
