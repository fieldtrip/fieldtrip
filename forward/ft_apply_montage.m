...
if ~isfield(montage, 'chantypeold')
  ...
  if isfield(input, 'chantype') && ~istrue(inverse)
    warning(defaultId, 'copying input chantype to montage');
...
if ~isfield(montage, 'chanunitold')
  ...
  if isfield(input, 'chanunit') && ~istrue(inverse)
    warning(defaultId, 'copying input chanunit to montage');
    [sel1, sel2] = match_str(montage.labelold, input.label);
...
if ~iscell(montage.labelold) || ~iscell(montage.labelnew)
  error(defaultId, 'montage labels need to be specified in cell-arrays');
end

% check the consistency of the montage
if ~all(isfield(montage, {'tra', 'labelold', 'labelnew'}))
  error(defaultId, 'the second input argument does not correspond to a montage');
end

% check the consistency of the montage
if size(montage.tra,1)~=length(montage.labelnew)
  error(defaultId, 'the number of channels in the montage is inconsistent');
...
if k > 0 && isfield(input, 'trial') % check for raw data now only
  ...
  if any(cellfun(hasnan, data_unused.trial))
    error(defaultId, 'FieldTrip:NaNsinInputData', ['Your input data contains NaNs in channels that are unused '...
...
if length(unique(montage.labelnew))~=m
  error(defaultId, 'not all output channels of the montage are unique');
end
if length(unique(montage.labelold))~=n
  error(defaultId, 'not all input channels of the montage are unique');
end
...
if length(intersect(inputlabel, montage.labelold))~=length(montage.labelold)
  error(defaultId, 'not all channels that are required in the montage are available in the data');
end
...
if isfield(input, 'chantype') && ~isequal(input.chantype, montage.chantypeold)
  error(defaultId, 'inconsistent chantype in data and montage');
...
switch inputtype
  ...
  case 'freq'
    ...
    elseif strcmp(freq.dimord, 'rpttap_chan_freq_time')
      ...
      error(defaultId, 'unsupported dimord in frequency data (%s)', freq.dimord);
    end
  ...
  otherwise
    error(defaultId, 'unrecognized input');
