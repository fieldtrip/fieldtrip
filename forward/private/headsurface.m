...
elseif ~isempty(headmodel) && isfield(headmodel, 'r') && length(headmodel.r)<5
  ...
  elseif length(headmodel.r)<5
    ...
    switch surface
      ...
      otherwise
        error(defaultId, 'other surfaces cannot be constructed this way');
    end
    if isfield(headmodel, 'o')
      origin = headmodel.o;
    else
...
elseif ft_voltype(headmodel, 'localspheres')
  ...
  if ~isfield(grad, 'tra') || ~isfield(grad, 'coilpos')
    error(defaultId, 'incorrect specification for the gradiometer array');
  end
  Nchans   = size(grad.tra, 1);
  Ncoils   = size(grad.tra, 2);
  Nspheres = size(headmodel.o, 1);
  if Nspheres~=Ncoils
    error(defaultId, 'there should be just as many spheres as coils');
  end
...
if ~isempty(inwardshift) && inwardshift~=0
  ...
  elseif tmp==-1
    ...
    warning(defaultId, 'cannot determine the orientation of the vertex normals');
    % nothing to do
  end
