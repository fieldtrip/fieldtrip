...
switch toolbox
  ...
  otherwise
    if ~silent, warning(defaultId, 'cannot determine whether the %s toolbox is present', toolbox); end
    dependency = false;
end
...
if isdeployed
  warning(defaultId, 'cannot change path settings for %s in a compiled application', toolbox);
  status = 1;
elseif exist(toolbox, 'dir')
  if ~silent
    ws = warning(defaultId, 'backtrace', 'off');
    warning(defaultId, 'adding %s toolbox to your MATLAB path', toolbox);
    warning(defaultId, ws); % return to the previous warning level
  end
...
try
  ...
  if strcmp(m.identifier, 'MATLAB:license:checkouterror')
    if nargin>1
      warning(defaultId, 'the %s toolbox is available, but you don''t have a license for it', toolbox);
    else
      warning(defaultId, 'the function ''%s'' is available, but you don''t have a license for it', funname);
    end
    status = false;
