function [ww, ix_eff_all, errTable_tr, errTable_te, parm, AXall,Ptr,Pte] =...
    biclsfy_slrlap(x_train, t_train, x_test, t_test, varargin)
% Binary classification by SLR with Laplace approximation (SLR-LAP).
% 
% -- Usage
...
% 2009/05/29 OY
% * The function is renamed from 'run_smlr_bi.m'.
...
if displaytext
    fprintf('------------------------------------\n');
    fprintf('  Binary classification by SLR-LAP  \n');
    fprintf('------------------------------------\n');
end
% add bias
if usebias == 1
...
[ww, ix_eff, W, AXall] = slr_learning(label, Xtr, @linfun,...
