...
% Netherlands.
%
% Input should be '*/*.ds' otherwise a list of stored recordings is made and used for input.
%
...
%% HANDLE INPUT
% add the lowest consistent path possible on Odin

% inventarise all recordings or use the input arguments
if nargin == 0
    recordings = [];
    ctr = 0;
    dirs = dir(['/home/action/arjsto' filesep 'MEG' filesep '*']); % list directories
    ndirs  = length(dirs);
    for d = 1:ndirs
        ds = dir(strcat(['/home/action/arjsto' filesep 'MEG' filesep, dirs(d).name, filesep '*.ds'])); % list recordings
        nds = length(ds);
        if ~isempty(ds)
            for r = 1:nds
                ctr = ctr +1;
                recordings{1,ctr} = ds(r).name; % file name
                recordings{2,ctr} = dirs(d).name; % accessory directory name
            end
        end
    end
    if isempty(recordings) % inventory check
        warning('No files with a ''.ds'' extension were found');
    end
elseif nargin >= 1 % use the input arguments
    recordings = [];
    ctr = 0;
    nds = length(varargin);
    for r = 1:nds
        check = findstr(varargin{r},'ds'); % check input for '.ds' extension
        if ~isempty(check)
            ctr = ctr +1;
            recordings{1,ctr} = varargin{r}; % file name
            check = [];
        end
    end
    if isempty(recordings) % inventory check
        warning('Input argument(s) should contain a ''.ds'' extension');
    end
end

%% CHECK PREVIOUS OUTPUT
% FIXME: make a check and then loop the remaining non-checked datasets


%% DATASET LOOP
nrecords = length(recordings);
for d = 1:nrecords
    
    %% DEFINE THE SEGMENTS
    tic
    % split the data in 10 second trials
    cfg                         = [];
    if size(recordings,1) == 1
        cfg.dataset = ['/home/action/arjsto' filesep 'MEG' filesep recordings{d}];
        fprintf(['selecting dataset ' recordings{1} '\n']);
    else
        cfg.dataset = ['/home/action/arjsto' filesep 'MEG' filesep recordings{2,d} filesep recordings{1,d}];
        fprintf(['selecting dataset ' recordings{2,d} filesep recordings{1,d} '\n']);
    end
