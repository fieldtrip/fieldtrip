...
if size(source.pos)~=size(fun,1)
  error(defaultId, 'inconsistent number of vertices in the cortical mesh');
end

if ~isfield(source, 'tri')
  error(defaultId, 'source.tri missing, this function requires a triangulated cortical sheet as source model');
end
...
if length(xparam)~=size(fun,2)
  error(defaultId, 'inconsistent size of "%s" compared to "%s"', cfg.funparameter, cfg.xparam);
end

if ~isempty(cfg.yparam)
  yparam = source.(cfg.yparam);
  if length(yparam)~=size(fun,3)
    error(defaultId, 'inconsistent size of "%s" compared to "%s"', cfg.funparameter, cfg.yparam);
  end
else
  yparam = [];
end
